...
static void print_return_error(uintptr_t pc, uintptr_t ret_pc/*R3*/){
     ...
     cout<<"stack top is: "<<local->top<<endl;
     if(local->top>=0) cout<<"addrs on stack is:"<<hex<<local->stack[local->top]<<endl;
...
generate_dynamic_events(JANUS_CONTEXT){
    ...
    first_pc = instr_get_app_pc(first_instr);
        for (id = 0; id < nmodules; ++id) {
            if (dr_module_contains_addr(loaded_modules[id], first_pc)) {
                matched = id;
                break;
            }
        }
      //  if(matched != -1 && strcmp(dr_module_preferred_name(loaded_modules[matched]), main_module) == 0){
        if((PCAddress)first_pc == 0x80490ae){
            dr_insert_clean_call(drcontext, bb, first_instr, (void *)count_BB, false, 1, OPND_CREATE_INT32(first_pc));
        }
#endif
#if FORWARD_CFI
...
generate_events_by_rule(JANUS_CONTEXT, instr_t *instr){
	...
	case VERIFY_JMP_TARGET:
            ...
            if(matched == -1) return;
#if TEST_HASH_LOOKUP
        dr_insert_clean_call(drcontext, bb, instr, (void *)verify_jmp_target_clean, false, 2, target_opnd, OPND_CREATE_INT32(matched));
#else
...
exit_summary() {
    ...
    std::ofstream outputFile("/local/scratch/ma843/rwdir-cfi/outerr-hash-all-fcfi.txt", std::ios_base::app);
