...
hashtable_t hashTable_IC[MAX_MODULES_ALLOWED];            //indirect call
hashtable_t hashTable_esym;            //indirect call
hashtable_t hashTable_IJ[MAX_MODULES_ALLOWED];            //indirect jump targets which are func entries
...
void verify_call_target_clean(uintptr_t target, int id){
      ...
      if(!hashtable_lookup(&hashTable_IC[id], (void*)target) && !hashtable_lookup(&hashTable_esym, (void*)target)){
          error_counter++;
#if DEBUG_CFI
          ...
          //   cout<<"valid tgt for icall - target: "<<hex<<target<<endl;
      }
#endif
...
void verify_jmp_target_clean(uintptr_t target, int id){
    ...
    if(!hashtable_lookup(&hashTable_IJ[id], (void*)target)){
        error_counter++;
#if DEBUG_CFI
...
#if DEBUG_CFI
    /*else{
         cout<<"valid tgt for ijmp in IJ -  tgt: "<<target<<endl;
    }*/
#endif
}
/*------------------------------------------------------------------------------*/
    ...
    while (binFile.read(reinterpret_cast<char*>(&address), sizeof(address))) {
        // Display the hex address
         //if (jc->functionMap.count(address)){
              ...
              hashtable_add(&hashTable_IC[id], (void*)address, (void*)1);
         //}
...
static void loadHashTables(const char * binname, int id, uintptr_t base_offset){
  ...
  /*indirect call target*/
    ...
    if(!str.empty()){
        ...
        hashtable_init(&hashTable_IC[id], HASH_KEY_SIZE, HASH_INTPTR, false);
        while (std::getline(infile, str)) {
          ...
          hashtable_add(&hashTable_IC[id],(void *)addr,(void*)1);
        }
    }
    infile.close();
    ...
    if(!str.empty()){
        ...
        hashtable_init(&hashTable_IJ[id], HASH_KEY_SIZE, HASH_INTPTR, false);
        while (std::getline(infile, str)) {
          std::istringstream iss(str);
          //addr = std::stoul(str);
          iss >>std::hex>>addr;
          ...
          hashtable_add(&hashTable_IJ[id],(void *)addr,(void*)1);
        }
        //hashTable_IJ[id]->insert(ld_addr, 1);
        hashtable_add(&hashTable_IJ[id],(void *)ld_addr,(void*)1);
    }
    infile.close();
  ...
  cout<<"STEP 3: filling exported symbols for "<<base_name<<endl;
    ...
    while (std::getline(infile, str)) {
        ...
        hashtable_add(&hashTable_esym,(void *)addr,(void*)1);
    }
    infile.close();
}
/*------------------------------------------------------------------------------*/
...
exit_summary() {
    ...
    std::ofstream outputFile("/local/scratch/ma843/rwdir-cfi/outerr-dummy.txt", std::ios_base::app);
    ...
    for(int i=0; i< nmodules; i++){
        hashtable_delete(&hashTable_IC[i]);
        hashtable_delete(&hashTable_IJ[i]);
        hashtable_delete(&hashTable_esym);
    }
}
...
event_module_load(void *drcontext, const module_data_t *info, bool loaded){
    ...
    if(strcmp(dr_module_preferred_name(info), "linux-gate.so.1") == 0) {
        ...
        hashtable_add(&hashTable_esym, (void*)vdso_addr, (void*)1);
    ...
    if(strcmp(dr_module_preferred_name(info), "libdynamorio.so") == 0) {
        ...
        hashtable_add(&hashTable_esym, (void*)DR_addr, (void*)1);
    }
    if(strcmp(dr_module_preferred_name(info), "ld-linux.so.2") == 0) {
        ...
        if(main_id != -1){
            ...
            hashtable_add(&hashTable_IJ[main_id], (void*)ld_addr, (void*)1);
        }
    }
...
#if DEBUG_CFI
    if(!start_BB){;
        if((uintptr_t)instr_get_app_pc(instrlist_last_app(bb)) == 0x8258171){
           dr_insert_clean_call(drcontext, bb, instrlist_first_app(bb), (void *)enable, false, 1, OPND_CREATE_INTPTR(bbAddr));
...
DR_EXPORT void dr_init(client_id_t id)
{
    ...
    /*for (int i = 0; i < MAX_MODULES_ALLOWED; i++) {
        hashTable_IC[i] = (hashtable_t*)malloc(sizeof(hashtable_t));
        hashTable_IJ[i] = (hashtable_t *)malloc(sizeof(hashtable_t));
    }
    hashTable_esym = (hashtable_t *)malloc(sizeof(hashtable_t));*/
//    hashTable_esym->init((int)ESYM_TABLE_SIZE);
    hashtable_init(&hashTable_esym, HASH_KEY_SIZE, HASH_INTPTR , false);
