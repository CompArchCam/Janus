...
static void print_verify_tgt();
stack_thread_t *shadowstack;
static void eerify_return_target(JANUS_CONTEXT, instr_t* trigger, uint64_t bitmask_flags, uint64_t bitmask_reg);
...
static void verify_jmp_target(JANUS_CONTEXT, instr_t *instr,uint64_t bitmask_flag, uint64_t bitmask_reg, opnd_t target_opnd, int id){
    ...
    if(inRegSet(bitmask_reg, R3)) dr_save_reg(drcontext, bb, instr,R3, SPILL_SLOT_2); //RCX, caller-saved
    if(inRegSet(bitmask_reg, R4)) dr_save_reg(drcontext, bb, instr,R4, SPILL_SLOT_3); //RDX, caller-saved
   ...
   if(bitmask_flag)
    ...
    if(bitmask_flag || inRegSet(bitmask_reg, DR_REG_XAX)){
    ...
    if(inRegSet(bitmask_reg, R4)) dr_restore_reg(drcontext, bb, instr,R4, SPILL_SLOT_3);
    if(inRegSet(bitmask_reg, R3)) dr_restore_reg(drcontext, bb, instr,R3, SPILL_SLOT_2);
}
static void verify_call_target(JANUS_CONTEXT, instr_t *instr,uint64_t bitmask_flag, uint64_t bitmask_reg, opnd_t target_opnd, int id){
    ...
    /*if(inRegSet(bitmask_reg, R1))*/ dr_save_reg(drcontext, bb, instr,R1, SPILL_SLOT_2); //EDI, for temporary data hold
    if(inRegSet(bitmask_reg, R3)) dr_save_reg(drcontext, bb, instr,R3, SPILL_SLOT_3); //RCX, caller-saved
    if(inRegSet(bitmask_reg, R4)) dr_save_reg(drcontext, bb, instr,R4, SPILL_SLOT_4); //RDX, caller-saved
//    dr_insert_clean_call(drcontext, bb, instr, (void *)print_tgt, false, 2,OPND_CREATE_INT32(instr_get_app_pc(instr)), target_opnd);
    dr_insert_clean_call(drcontext, bb, instr, (void *)print_verify_tgt, false,0);
   //load target operand value before saving arith flags 
    ...
    if(bitmask_flag)
        dr_restore_arith_flags(drcontext, bb, instr, SPILL_SLOT_5);
    if(inRegSet(bitmask_reg, R4)) dr_restore_reg(drcontext, bb, instr,R4, SPILL_SLOT_4);
    if(inRegSet(bitmask_reg, R3)) dr_restore_reg(drcontext, bb, instr,R3, SPILL_SLOT_3);
    /*if(inRegSet(bitmask_reg, R1))*/ dr_restore_reg(drcontext, bb, instr,R1, SPILL_SLOT_2);
}
static void verify_return_target(JANUS_CONTEXT, instr_t *instr,uint64_t bitmask_flag, uint64_t bitmask_reg){
    ...
    /*** save context*****/
    if(inRegSet(bitmask_reg, R1)) dr_save_reg(drcontext, bb, instr,R1, SPILL_SLOT_2);
    if(inRegSet(bitmask_reg, R2)) dr_save_reg(drcontext, bb, instr, R2, SPILL_SLOT_3);
    if(inRegSet(bitmask_reg, R3)) dr_save_reg(drcontext, bb, instr, R3, SPILL_SLOT_4);
    if(inRegSet(bitmask_reg, R4)) dr_save_reg(drcontext, bb, instr, R4, SPILL_SLOT_6);
    if(bitmask_flag)
        dr_save_arith_flags(drcontext, bb, instr, SPILL_SLOT_5);
   ...
   //  increment error counter
    ...
    if(bitmask_flag)
        dr_restore_arith_flags(drcontext, bb, instr, SPILL_SLOT_5);
    if(inRegSet(bitmask_reg, R4)) dr_restore_reg(drcontext, bb, instr, R4, SPILL_SLOT_6);
    if(inRegSet(bitmask_reg, R3)) dr_restore_reg(drcontext, bb, instr, R3, SPILL_SLOT_4);
    if(inRegSet(bitmask_reg, R2)) dr_restore_reg(drcontext, bb, instr, R2, SPILL_SLOT_3);
    if(inRegSet(bitmask_reg, R1)) dr_restore_reg(drcontext, bb, instr, R1, SPILL_SLOT_2);
...
generate_events_by_rule(JANUS_CONTEXT, instr_t *instr){
	...
	break;
       ...
       case VERIFY_CALL_TARGET_INTRA:
            ...
            verify_call_target(janus_context, trigger, flag_live_on ? rule->reg0 :  1/*bitmask_flags*/, reg_live_on ? rule->reg1 : 0/*bitmask_reg*/, target_opnd, matched);
	...
	case VERIFY_JMP_TARGET:
            ...
            verify_jmp_target(janus_context, trigger,  flag_live_on ? rule->reg0 : 1/*bitmask_flags*/, reg_live_on ? rule->reg1 : 0/*bitmask_reg*/, target_opnd, matched);
	break;
#endif
         /*case SAVE_AT_ENTRY:
             if(monitor_enable && reg_live_on){
                if(rule->reg0)//save_ecx = 1
                    PRE_INSERT(bb, trigger,
                        INSTR_CREATE_mov_st(drcontext,
                                        opnd_create_abs_addr(&(regs.slot1), OPSZ_4),
                                        opnd_create_reg(DR_REG_ECX)));
                if(rule->reg1)//save_edx = 1
                    PRE_INSERT(bb, trigger,
                        INSTR_CREATE_mov_st(drcontext,
                                    opnd_create_abs_addr(&(regs.slot2), OPSZ_4),
                                    opnd_create_reg(DR_REG_EDX)));
            }
        break;
        case RESTORE_AT_EXIT:
            if(monitor_enable && reg_live_on){
                if(rule->reg0)//restore_ecx = 1
                    PRE_INSERT(bb, trigger,
                        INSTR_CREATE_mov_ld(drcontext, opnd_create_reg(DR_REG_ECX),
                                            opnd_create_abs_addr(&(regs.slot1), OPSZ_4)));
                if(rule->reg1)//restore_edx = 1
                    PRE_INSERT(bb, trigger,
                        INSTR_CREATE_mov_ld(drcontext,
                                        opnd_create_reg(DR_REG_EDX),
                                    opnd_create_abs_addr(&(regs.slot2), OPSZ_4)));
...
static void print_BB(uintptr_t BBaddr){
    ...
    for (id = 0; id < nmodules; ++id) {
        if (dr_module_contains_addr(loaded_modules[id],(app_pc)BBaddr)) {
            ...
            if((uintptr_t)loaded_modules[id]->start == KEYBASE)
                cout<<"BB: "<<hex<<(PCAddress)BBaddr<<" from "<<loaded_modules[id]->full_path<<endl;
            else
                cout<<"BB: "<<hex<<(PCAddress)BBaddr-(PCAddress)loaded_modules[id]->start<<" from "<<loaded_modules[id]->full_path<<endl;
            break;
        }
    }
    bb_count++;
    if(bb_count == 10031) exit(0);
}
static void print_verify_tgt(){
    ...
    for (id = 0; id < nmodules; ++id) {
        if (dr_module_contains_addr(loaded_modules[id],(app_pc)BBaddr)) {
            //cout<<"BB: "<<hex<<(uintptr_t)BBaddr<<" real:"<<(PCAddress)BBaddr-(PCAddress)loaded_modules[id]->start<<" from "<<loaded_modules[id]->full_path<<endl;;
            cout<<"instr: "<<hex<<(PCAddress)BBaddr-(PCAddress)loaded_modules[id]->start<<" from "<<loaded_modules[id]->full_path<<endl;;
            break;
        }
    }*/
                  ...
                  bool for_trace, bool translating)
{
    ...
    dr_insert_clean_call(drcontext, bb, instrlist_first_app(bb), (void *)print_BB, false, 1,OPND_CREATE_INT32(bbAddr));     
...
DR_EXPORT void dr_init(client_id_t id)
{
    ...
    dr_fprintf(STDOUT,"---------------------------------------------------------------\n\n");
#endif
    set_client_mode((JMode)JCFI_LIVE);
