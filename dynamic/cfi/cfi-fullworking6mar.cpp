...
exit_summary() {
    ...
    //std::ofstream outputFile("/local/scratch/ma843/rwdir-cfi/outerr-cfi-inlined-full.txt", std::ios_base::app);
...
event_module_load(void *drcontext, const module_data_t *info, bool loaded){
    ...
    if(strcmp(dr_module_preferred_name(info), "ld-linux.so.2") == 0) {
        ...
        /*if(main_id != -1){
            cout<<"added ADDR: "<<hex<<ld_addr<<endl;
             //hashTable_IJ[main_id]->insert(ld_addr, 1);
             insert(&hashTable_IJ[main_id], ld_addr, 1);
        }*/
