...
//    dr_insert_clean_call(drcontext, bb, instr, (void *)print_tgt, false, 2,OPND_CREATE_INT32(instr_get_app_pc(instr)), target_opnd);
   //load target operand value before saving arith flags 
    ...
    /*----START -- new code to save esym in cache------*/
      //check in esym_cache which is initially 0.  
    //if not found in IC table, check in esym_cache, and then if not found, check in esym_table
    meta_instr = INSTR_CREATE_mov_ld(drcontext,opnd_create_reg(R3), opnd_create_abs_addr(&esym_cache, OPSZ_4));
    instrlist_meta_preinsert(bb, instr, meta_instr);
    
    meta_instr = INSTR_CREATE_cmp(drcontext,opnd_create_reg(R1),opnd_create_reg(R3));
    instrlist_meta_preinsert(bb, instr,meta_instr);

    meta_instr = INSTR_CREATE_jcc(drcontext,OP_jz,opnd_create_instr(restore_label));
    instrlist_meta_preinsert(bb, instr, meta_instr);

    /*----END -- new code to save esym in cache------*/
    ...
    /*----START -- new code to save esym in cache------*/
       //if found, save it in esym_cache. else, go to restore
    meta_instr = INSTR_CREATE_jcc(drcontext,OP_jz,opnd_create_instr(error_label));
    instrlist_meta_preinsert(bb, instr, meta_instr);
    
    meta_instr = INSTR_CREATE_mov_st(drcontext,opnd_create_abs_addr(&esym_cache, OPSZ_4), opnd_create_reg(R1));
    instrlist_meta_preinsert(bb, instr, meta_instr);
...
exit_summary() {
    ...
    std::ofstream outputFile("/local/scratch/ma843/rwdir-cfi/outerr-cfi-esymcache-newhash.txt", std::ios_base::app);
