...
/*------------------------------------------------------------------------------*/
#if DEBUG_VERBOSE
void store_return_target_clean(uintptr_t addr, uintptr_t instr_pc, void *stack)
#else
void store_return_target_clean(uintptr_t addr, void * stack)
#endif
{
    stack_thread_t *local  = (stack_thread_t*)stack;
...
#if DEBUG_VERBOSE
void verify_longjmp_target_clean(uintptr_t rtn_addr_on_stack, uintptr_t instr_pc, void *stack)
#else
void verify_longjmp_target_clean(uintptr_t rtn_addr_on_stack, void * stack)
#endif
{
#if DEBUG_VERBOSE
...
static void verify_return_target(){
...
#if 0
static void store_return_target(JANUS_CONTEXT, instr_t *instr, int size,uint64_t bitmask_flag, uint64_t bitmask_reg){
    opnd_t mem_operand; 
    ...
    MOV %ecx, (%eax) # shadow stack via ECX
      */  
    dr_insert_get_seg_base(drcontext, bb, instr, (reg_id_t)DR_SEG_GS, (reg_id_t)DR_REG_RDI);
    meta_instr = INSTR_CREATE_xor(drcontext, opnd_create_reg(DR_REG_R8), OPND_CREATE_MEM64(DR_REG_RDI, 108));
...
generate_dynamic_events(JANUS_CONTEXT){
    ...
    if(instr_is_call(last)){
...
#if DEBUG_VERBOSE
         dr_insert_clean_call(drcontext, bb, last, (void *)store_return_target_clean, false, 3, OPND_CREATE_INTPTR((PCAddress)call_pc + size), OPND_CREATE_INTPTR(call_pc), OPND_CREATE_INTPTR((stack_thread_t*)dr_get_tls_field(dr_get_current_drcontext())));
#else
         dr_insert_clean_call(drcontext, bb, last, (void *)store_return_target_clean, false, 2, OPND_CREATE_INTPTR((PCAddress)call_pc + size), OPND_CREATE_INTPTR((stack_thread_t*)dr_get_tls_field(dr_get_current_drcontext())));
#endif
    }
         ...
         dr_insert_clean_call(drcontext, bb, last, (void *)verify_longjmp_target_clean, false, 3, OPND_CREATE_MEMPTR(DR_REG_XSP,0), OPND_CREATE_INTPTR(instr_get_app_pc(last)), OPND_CREATE_INTPTR((stack_thread_t*)dr_get_tls_field(dr_get_current_drcontext())));
#else
         dr_insert_clean_call(drcontext, bb, last, (void *)verify_longjmp_target_clean, false, 2, OPND_CREATE_MEMPTR(DR_REG_XSP,0), OPND_CREATE_INTPTR((stack_thread_t*)dr_get_tls_field(dr_get_current_drcontext())));
#endif
    }
    return;
}
...
generate_events_by_rule(JANUS_CONTEXT, instr_t *instr){
...
#endif
#if DEBUG_VERBOSE
                dr_insert_clean_call(drcontext, bb, trigger, (void *)store_return_target_clean, false, 3, OPND_CREATE_INTPTR((PCAddress)instr_get_app_pc(instr) + rule->reg2), OPND_CREATE_INTPTR(instr_get_app_pc(trigger)), OPND_CREATE_INTPTR((stack_thread_t*)dr_get_tls_field(dr_get_current_drcontext())));
#else
                dr_insert_clean_call(drcontext, bb, trigger, (void *)store_return_target_clean, false, 2, OPND_CREATE_INTPTR((PCAddress)instr_get_app_pc(instr) + rule->reg2), OPND_CREATE_INTPTR((stack_thread_t*)dr_get_tls_field(dr_get_current_drcontext())));
#endif
            }
	break;
	...
	case VERIFY_RETURN_TARGET:
            if(monitor_enable){
                ...
                dr_insert_clean_call(drcontext, bb, trigger, (void *)verify_longjmp_target_clean, false, 3, OPND_CREATE_MEMPTR(DR_REG_XSP,0), OPND_CREATE_INTPTR(instr_get_app_pc(trigger)), OPND_CREATE_INTPTR((stack_thread_t*)dr_get_tls_field(dr_get_current_drcontext())));
#else
                dr_insert_clean_call(drcontext, bb, trigger, (void *)verify_longjmp_target_clean, false, 2, OPND_CREATE_MEMPTR(DR_REG_XSP,0), OPND_CREATE_INTPTR((stack_thread_t*)dr_get_tls_field(dr_get_current_drcontext())));
#endif
                }
