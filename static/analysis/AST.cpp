...
void Function::printASTDot(void *outputStream)
{
    ...
    for (auto expr : exprs) {
        ...
        switch (expr->kind) {
        case Expr::MEM:
            os << "\tE" << hex << (uint64_t)(expr->m) << " -> E"
               << (uint64_t)(expr) << endl;
            break;
        case Expr::BINARY:
            os << "\tE" << hex << (uint64_t)(expr->b.e1) << " -> E"
               << (uint64_t)(expr) << endl;
            os << "\tE" << hex << (uint64_t)(expr->b.e2) << " -> E"
               << (uint64_t)(expr) << endl;
            break;
        case Expr::PHI:
            os << "\tE" << hex << (uint64_t)(expr->p.e1) << " -> E"
               << (uint64_t)(expr) << endl;
            os << "\tE" << hex << (uint64_t)(expr->p.e2) << " -> E"
               << (uint64_t)(expr) << endl;
            break;
        case Expr::UNARY:
            os << "\tE" << hex << (uint64_t)(expr->u.e) << " -> E"
               << (uint64_t)(expr) << endl;
            break;
        default:
            break;
