...
void printSSADot(janus::Function &function, void *outputStream) {
    ...
    for (auto vs: function.allStates) {
        if (vs->notUsed) continue;
        if (vs->isPHI) {
            os<<"V"<<dec<<vs->id<<" [label=\""<<vs<<" in BB"<<dec<<vs->block->bid<<"\"";
            if (vs->type == JVAR_STACK || vs->type == JVAR_STACKFRAME) os<<",fillcolor=dodgerblue";
            else os<<",fillcolor=cyan";
    ...
    for (auto &instr: function.instrs) {
        if (instr.opcode == Instruction::Nop) continue;
        else if (instr.opcode == Instruction::Call) {
            os<<"I"<<dec<<instr.id<<" [label=\""<<instr<<"\",shape=\"box\",fillcolor=crimson, fontcolor=white];"<<endl;
        }
        else
            os<<"I"<<dec<<instr.id<<" [label=\""<<instr<<"\",shape=\"box\",fillcolor=gold];"<<endl;
    }
...
void printSSADot(janus::Loop &loop, void *outputStream)
{
    ...
    os << "label=\"Loop " << loop.id << " SSA graph in function:"<<loop.parent->name<<"\";" << endl;
    ...
    for (auto vs: loopNodeExpanded) {
        ...
        if (vs->isPHI) {
            ...
            else if (vs->type == JVAR_STACK || vs->type == JVAR_STACKFRAME) os<<",fillcolor=dodgerblue";
            else os<<",fillcolor=cyan";
            os<<",shape=\"diamond\"];"<<endl;
        } else if (vs->type == JVAR_MEMORY) {
            os<<"V"<<dec<<vs->id<<" [label=\""<<vs<<"\",fillcolor=indigo,fontcolor=white,shape=box];"<<endl;
            ...
            os<<"V"<<dec<<vs->id<<" [label=\""<<vs<<"\"";
            if (vs->type == JVAR_STACK || vs->type == JVAR_STACKFRAME) os<<",fillcolor=dodgerblue";
            else if (loop.isConstant(vs)) os<<",fillcolor=white";
            os<<",shape=\"box\"];"<<endl;
        }
    }
        ...
        for (int i=0; i<bb.size; i++) {
            ...
            for (auto cd: instr.controlDep) {
                ...
                if (loop.contains(cd->block->bid))
                    os<<"I"<<dec<<cd->id<<" -> I"<<instr.id<<" [style=\"dashed\", color=crimson];"<<endl;
            }
        }
    }
#endif
...
generateExeReport(JanusContext *context, void *outputStream)
{
    ...
    for (auto &loop: context->loops) {
        ...
        if (loop.memoryDependences.size() && loop.undecidedMemAccesses.size()==0)
        ...
        if (shared_call_found) num_loops_with_shared_call++;
    }
    report<<"\tNo. of recognised loop nests: "<<num_loops_nests<<endl;
    report<<"\tNo. of loops that could not be parallelised by Janus "<<num_unsafe_loops<<endl;
    report<<"\tNo. of loops with recognised loop iterators "<<num_loops_with_iter<<endl;
    report<<"\tNo. of loops with ambiguous memory accesses "<<num_undecided_loops<<endl;
    report<<"\tNo. of loops with clear static dependencies "<<num_loops_with_static_dep<<endl;
    ...
    report<<"\tNo. of loops that requires profiling "<<num_profile_loops<<endl;
}
