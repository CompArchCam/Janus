...
#ifdef DDG
static void loadDDG(JanusContext *gc, Loop &loop, char *rawDDG, uint32_t size);
#endif
...
void loadLoopCoverageProfiles(JanusContext *gc)
{
    ...
    if (lcov.good()) {
        GSTEPCONT("\tReading " << gc->name << ".lcov for coverage filtering"
                               << endl);
        ...
        float coverage;
        string name;
        while (lcov >> loopID >> coverage >> invocation >> iteration >>
               inaccurate >> name) {
            ...
            // cout<<loopID<<" "<<coverage<<" "<<invocation<<" "<<iteration<<"
            // "<<name<<endl;
        }
    }
    lcov.close();
...
void filterParallelisableLoop(JanusContext *gc)
{
    for (auto &loop : gc->loops) {
        if (loop.coverage > JANUS_LOOP_COVERAGE_THRESHOLD &&
            ...
            (loop.total_iteration_count / loop.invocation_count) >
                JANUS_LOOP_MIN_ITER_COUNT) {
            cout << "\t" << loop.id << " " << loop.coverage << " "
                 << (loop.total_iteration_count / loop.invocation_count)
                 << endl;
            loop.removed = false;
        } else
...
void loadLoopSelection(JanusContext *jc)
{
    ...
    if (!iselect.good())
        return;
    GSTEPCONT("\tReading " << jc->name << ".loop.select for loop selection"
                           << endl);
...
void loadDDGProfile(JanusContext *gc)
{
    ...
    if (ifddg.good()) {
        GSTEPCONT("\tReading " << gc->name
                               << ".loop.ddg for dependence analysis" << endl);
    ...
    for (auto &loop : gc->loops) {
        if (gc->manualLoopSelection && !loop.pass)
            continue;
...
static void loadDDG(JanusContext *gc, Loop &loop, char *rawDDG, uint32_t size)
{
    ...
    DDG &ddg = *loop.pDDG;
    ddg.edges = (DDGEdge *)(rawDDG + sizeof(DDGHeader) +
