...
void scanMemoryAccess(janus::Function *function);

void scanDefUse(janus::Function *function);

void livenessAnalysis(janus::Function *function);

void reachingAnalysis(janus::Function *function);
...
/* Get all instructions which have reaching definations on the current
 * instruction the instruction must be in the basic block */
void getReachingInstructionList(std::set<InstID> &reachingSet,
                                janus::MachineInstruction &instr,
                                janus::Variable var);
