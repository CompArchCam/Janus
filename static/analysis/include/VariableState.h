 ...
 *
 * The **VariableState** class is used to represent a single variable in SSA Form. We represent the SSA form as a graph for each function.
 * We can have the following types of nodes (variables):
 *
 * - Normal variables (*assignments*) are defined in the code as the output of some operation. Assignment nodes have their *lastModified* field set to point to the **MachineInstruction** object that defined them.
 * - *Phi nodes* represent junctions, they contain a list of possible predecessors in the field *phi*. They are guaranteed to have more than one entry in *phi*. All phi nodes are bound to the **start** of the basic block where the different definitions first merge (not to the block where the new definition is first used).
 * - *Memory operands* represent the input address to a load instruction. If the address is given in [base+offset] form, then they are linked through *phi* to the base register (1 element), otherwise *phi* is empty. The *Variable::isLoad()* function of their variable (getVar()) returns *true*.
 * - Any other variable is an *initial value*. These are the inputs to the function or uninitialized registers that are used as inputs to some instruction.
 */
class VariableState {
    public:
    //var
     ...
     */
    bool isInduction;
    /** \brief Set to the loop in which this variable is a secondary induction variable
     *
     *  A secondary induction variable is a temporary variable that is calculated only
     *  from induction variables and constant variables.
     *
     *  Unlike for induction variables no additional fields (such as repInductionVar,
     *  constDifference, update) are set at the moment.
     */
    Loop* secondaryInductionLoop = NULL;
    /** \brief Pointer to the Loop in which this variable is an induction variable.
     *
     * inductionLoop!=NULL if and only if it is an induction variable increased by upd in every 
     * iteration of the loop pointed to.
     *
     * Note, that this loop is well-defined, and the variable is necessarily constant in any
     * nested loops.
     */
    Loop *inductionLoop = NULL;
    /** \brief Representative element of a group of equivalent induction variables.
     *
     * Two variables are in the same group iff their updates are confirmed to be equivalent, 
     * hence making them equal in every iteration up to a constant difference. Each variable 
     * within the group has this field point to the same value.
     *
     * Eg in the loop for(int i=0, j=1; i<100; i++,j++) {}, i and j are equivalent up to the
     *    constant difference of 1.
     *
     * Currently only induction variables discovered together are considered equal, ie only those
     * that are connected via assignments.
     */
    VariableState *repInductionVar = NULL;
    /** \brief The constant difference from repInductionVar.
     *
     * This SSA variable can be obtained from repInductionVar by applying constDifference.
     */
    UpdateSequence *constDifference = NULL;

    /** \brief Value added in each iteration.
     * 
     * If inductionLoop!=NULL, then upd contains the update sequence for this variable.
     * This non-zero amount is added to the variable in every iteration.
     * 
    ...
    /** \brief The variable state is not used as inputs for its subsequent instructions.
     *
     * This field is true if the variable state or its subsequent states are not used by downstream instructions.
     * It means the information stored in this state can be used as scratch space. 
     */
    bool notUsed;
     ...
     *
     * This field is intended to be accessed through *Loop::isConstant(VariableState\*)*, 
    ...
    /** \brief This points to a state equal to this, or itself if it is not constant in any loops.
     *
     ...
     *
     * Finds the element r = equalsTo->equalsTo->... such that r->equalsTo==r. r is called the 
     * representative element of the group.
     *
     * Sideeffect: compresses the path (links equalsTo of all states along the path to r)
     *
     * Complexity: O(n). Amortized O(1) per query for a large number of queries.
     */
    VariableState *rep();

    /** \brief Returns whether two states (constant in the same loop) are considered equal. */
    bool equals(VariableState *oth);

    /** \brief Returns the type of variable that this state is. */
    VariableStateType type();

    /** \brief Marks this *VariableState* object as a recognized stack-based variable.
     *
     ...
     *
     * If this is a *normal variable*, then lastModified contains the instruction defining it.
     * For any of the other types it is set to *NULL*;
     */ 
     ...
     *
     * Pointers to all states that can directly inherit the value of this state. States in 
     * dependants are necessarily phi nodes or memory operands.
     */
    std::vector<VariableState*> linksToPhi;

    /** \brief Constructs a normal variable state */
    VariableState(Variable var, BasicBlock *block, MachineInstruction* lastModified);
    /** \brief Constructs a phi node or memory operand with phi=*vec
     *
     * No copying is done, so vec must not go out of scope, be deleted manually or reused 
     * (modified).
     */
    VariableState(Variable var, BasicBlock *block, std::vector<VariableState*> *vec); //phi node
    /** \brief Constructs a variable state holding an initial value.
     *
     * This is also the constructor to be used to initialize an "empty" object, which can be 
     * filled later.
     */
