...
/**  \brief A SCScale class represents either an integer scale or a variable
 * scale.
 *
 ...
 *
 *   Currently **SCExpr** is a summation of **Variable** times **SCScale**
 * together. expr = v1 * s1 + v2 * s2 + v3 * s3 ... + C where v are
 * **VariableState** and s are **SCScale**
 */
class SCExpr
{
  public:
    ...
    std::string print();
    /// Return the value of loop's induction variables if the expression is
    /// evaluated as zero. Return -1 if failed.
    ...
    bool isConstant();
    /// Evaluate the maximum value of this expression, return -1 if not able to
    /// evaluate
...
/**  \brief A ScalarEvolution class represents a 3-item tuple: {base, op,
 * stride} \@loop
 *
 *  The **ScalarEvolution** class is used to represent the range of an array
 * access in terms of {base, +, stride} \@loop.
 *
 *  The value of *v* can be evaluated as base + indvar * stride and indvar is
 * the induction variable of the ScalarEvolution::loop . */
class ScalarEvolution
{
  ...
  public:
    ...
    SCExpr varExpr;
    /// The stride expression for this variable (the scale of the induction
    /// variable)
    SCExpr strideExpr;
...
bool buildInductionStrideExpression(janus::VariableState *vs,
                                    janus::VariableState *induct,
                                    janus::SCScale &stride);

/// construct a sclar expression in terms of the loop induction variable
bool buildSCExpr(janus::VariableState *vs, janus::SCExpr &expr,
                 janus::Loop *loop);
