...
void aliasAnalysis(janus::Loop *loop, std::vector<janus::Loop>& allLoops, std::vector<std::set<LoopID>>& loopNests)
{
    ...
    IF_LOOPLOG2(
    for (auto memLoc: readSets) {
        loopLog2 <<"\t\t"<<*memLoc<<endl;
    });
    LOOPLOG2("\t"<<"Identified write set: "<<endl);
    IF_LOOPLOG2(
    for (auto memLoc: writeSets) {
        loopLog2 <<"\t\t"<<*memLoc<<endl;
    });
    ...
    for (auto memWrite: writeSets) {
        ...
        if (!memWrite->containIterator(loop)) {
            LOOPLOG2("\t\tFound a write after write dependency. \n\t\tIt could be resolved by privatizing memories :"<<*memWrite<<endl);
    ...
    IF_LOOPLOG2(
    if (loop->memoryDependences.size()==0 && loop->undecidedMemAccesses.size()==0) {
        loopLog2<<"\nThis loop is a DOALL loop ";
        if (loop->arrayToCheck.size()*(loop->arrayAccesses.size()-1))
            loopLog2<<"with runtime array base check of "<<loop->arrayToCheck.size()*(loop->arrayAccesses.size()-1)<<endl;
        else loopLog2<<"!"<<endl;
    });
}
    ...
    if (m1.escev && m2.escev) {
        ...
        if (e1.strides.size()==1 &&
            ...
            if (result == MustAlias) {
                ...
                return;
            } else if (result == MustNotAlias) return;
        }
        ...
        loop->undecidedMemAccesses.insert(&m1);
        LOOPLOG2("\t\t\tCurrently janus is not able to find the dependencies relation for this pair"<<endl);
    } else {
        ...
        if (eval.first == Expr::INTEGER) {
            ...
            if (diffImm != 0) {
                if (loop->mainIterator &&
                    ...
                    int64_t max_bound = loop->mainIterator->stride * loop->mainIterator->step;
                    if (abs(diffImm) >= abs(max_bound)) {
                        LOOPLOG2("\t\t\tVector difference is beyond the loop boundary "<<loop->mainIterator->stride * loop->mainIterator->step<<" for this write-read pair, therefore no cross-iteration dependence"<<endl);
                        return;
                    }
                }
                LOOPLOG2("\t\t\tFound a cross iteration dependence for this write-read pair"<<endl);
                loop->memoryDependences[&m1].insert(&m2);
            } else {
                LOOPLOG2("\t\t\tInternal uses for this write-read pair"<<endl);
            }
        } else {
            if (diff.exprs.size()) {
                loop->undecidedMemAccesses.insert(&m1);
                ...
                LOOPLOG2("\t\t\tFound an exact self-dependence for this write-read pair, it is safe for parallelisation"<<endl);
            }
        }
    }
}
...
static AliasType GCDTest(ExpandedSCEV &e1, ExpandedSCEV &e2, ExpandedSCEV &diff, Loop &loop)
{
    ...
    for (auto term: e1.strides) {
        if (term.second.kind == Expr::INTEGER) {
            if (gcd == -1) gcd = term.second.i;
            else gcd = get_gcd(gcd, term.second.i);
        } else return UnknownAlias;
    }
...
ExpandedSCEV getRangeDiff(ExpandedSCEV &e1, ExpandedSCEV &e2, janus::Loop *loop)
{
    ...
    for (auto i2: e2.strides) {
        ...
        if (diff.strides.find(iter) != diff.strides.end()) {
            //if the iterator is parent loops, perform subtraction, else keep the iterator
...
int
ExpandedSCEV::evaluate1StepDistance(Loop *loop)
{
    int result = 0;
...
{
    for (auto s: strides) {
        if (s.first->loop == loop) return s.first;
    }
}

static void
buildSCEV(SCEV *scev, ExpandedExpr &expr, Loop *loop)
{
    for (auto e: expr.exprs) {
        ...
        if (term.iteratorTo) {
            ...
            if (term.iteratorTo->loop == loop) {
                ...
                for (auto e=init.ee->exprs.begin(); e!=init.ee->exprs.end();) {
                    Expr node = (*e).first;
                    Expr coeff = (*e).second;
                ...
                if (temp.size() > 1) {
                    LOOPLOG2("\t\t\tMultiple iterator in SCEV "<<temp<<" not yet implemented"<<endl);
                    return;
                } else if (temp.size() == 1) {
...
getOrInsertMemLocations(Loop *loop, MemoryLocation *loc)
{
    ...
    if (query == loop->arrayAccesses.end()) {
        ...
        location = loc;
        LOOPLOG2("\t\t\tInsert new array base ("<<arrayBase<<") with location "<<*location<<endl);
    } else {
        ...
        for (auto &l: locations) {
            if (l && (*l == *loc)) {
                location = l;
                LOOPLOG2("\t\t\tFound existing array base ("<<arrayBase<<") with existing location "<<*location<<endl);
                delete loc;
                return location;
            }
        }
        if (!location) {
            ...
            location = loc;
            LOOPLOG2("\t\t\tFound existing array base ("<<arrayBase<<") with new location "<<*location<<endl);
        }
    }
    return location;
}
