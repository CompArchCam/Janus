...
namespace janus {
    ...
    enum MachineInstCoarseType
    {
        ...
        INSN_PREINDEX = 32,  //5. instructions using the preindex addressing mode (AArch64)
        INSN_POSTINDEX = 64, //6. instructions using the postindex addressing mode (AArch64)
        /* NOTE: no instruction can use post and pre indexing (currently) so if this was redesigned
             for AArch64 they should be mutually exclusive */
    };
    ...
    public: 
        ...
        */
        ...
        /* bool                            isXMMInstruction(); - Not relevant for ARM. Could instead check
                                                                    for ARM NEON SIMD instructions if needed. */
