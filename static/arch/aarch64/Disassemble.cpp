...
// Pre- and post-indexing
    ...
    case ARM64_INS_BLR: {
        Variable var(
            (uint32_t)
                JREG_X0); /* TODO It appears to be more complicated than this on
                             ARM as return values may be passed in any of R0-R7
                             and pointers to memory areas appear to be placed in
                             R8. Also, you can also pass/return using V0-V7 when
                             doing SIMD/vector stuff. */
        VarState *vs = new VarState(var, block, &instr);
        function->allStates.insert(vs);
        instr.outputs.push_back(vs);
    } break;
