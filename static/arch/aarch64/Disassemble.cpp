...
static void liftInstruction(Instruction &instr, Function *function)
{
    ...
    switch(minstr->opcode)
    {
        case ARM64_INS_CMP: //fairly sure CMP only writes control flags but capstone seems to think it writes a register
            // No reads/writes to registers (other than control flags but this is checked elsewhere)
        break;
        ...
        case ARM64_INS_STXR:
            ...
            {
                var = minstr->operands[i].lift(instr.pc);
                //record this variable
                function->allVars.insert(var);

                //Only add memory outputs
                if (minstr->operands[i].access == OPND_WRITE
                        || (minstr->operands[i].access == OPND_READ_WRITE && minstr->operands[i].type == OPND_MEM)) {
...
//Pre- and post-indexing
                ...
                if((minstr->isType(INSN_PREINDEX) || minstr->isType(INSN_POSTINDEX)) &&
                    ...
                    // For pre/post indexed addresses, base register is written back to
                    var = Variable((uint32_t)(minstr->operands[i].mem.base));
                    vs = new VarState(var, block, &instr);
                    function->allStates.insert(vs);
                    instr.outputs.push_back(vs);
                }
        ...
        case ARM64_INS_STP:
            for(int i=0; i<minstr->opndCount; i++)
            {
                ...
                if (minstr->operands[i].access == OPND_WRITE
                        || (minstr->operands[i].access == OPND_READ_WRITE && minstr->operands[i].type == OPND_MEM)) {
                    //create two variable states for stp
                    vs = new VarState(var, block, &instr);
                    instr.outputs.push_back(vs);
...
//Pre- and post-indexing
    ...
    switch(minstr->opcode)
    {
        ...
        case ARM64_INS_BLR:
        {
            Variable var((uint32_t)JREG_X0); /* TODO It appears to be more complicated than this on ARM
                                                 as return values may be passed in any of R0-R7 and
                                                 pointers to memory areas appear to be placed in R8.
                                                 Also, you can also pass/return using V0-V7 when doing
                                                 SIMD/vector stuff. */
            VarState *vs = new VarState(var, block, &instr);
            function->allStates.insert(vs);
            instr.outputs.push_back(vs);
        ...
        case ARM64_INS_LDXR:
            ...
            for(int i=0; i<minstr->opndCount; i++)
            {
                    ...
                    (minstr->operands[i].access == OPND_READ_WRITE && minstr->operands[i].type == OPND_MEM))
                {
                    ...
                    if (minstr->operands[i].type == OPND_MEM && minstr->opcode == ARM64_INS_LDP)
                    {   // Add an additional implicit memory operand for load pair instructions
                        var.value += minstr->operands[i].size;
                        v.push_back(var);    
                    }
                }
// Pre- and post-indexing
        ...
        //Instructions for which we ignore READ_WRITE operands because they're basically just WRITE operands
        case ARM64_INS_MOVZ:
        case ARM64_INS_MOVI:
        case ARM64_INS_MOVN:
        case ARM64_INS_SXTB:
...
void linkArchSpecSSANodes(janus::Function &function, map<BlockID, map<Variable, VarState*> *> &globalDefs)
{
...
propagateX29(VarState *vs, int64_t offset, map<BlockID, map<Variable, VarState*> *> globalDefs, Function& function)
{
    ...
    if (vs->type == JVAR_MEMORY) {
        ...
        if(vs->value == JREG_SP || vs->value == JREG_WSP) {
            propagationValid = true; /* Perform no transformations on SP related
                ...
                for (int i = 0; i < bb.size; i++)
                {
                    ...
                    for (auto vi : bb.instrs[i].inputs)
                    {
                        if (vi == vs) { //Deliberate ptr equality test
                            propagationValid &= propagateX29Instr(&bb.instrs[i], vs, offset, globalDefs, function);
                        }
                        else if (vi->type == JVAR_MEMORY && vi->base == vs->value) {
                            if(vi->index != JREG_NULL && vi->index != JREG_INVALID) {
                                propagationValid = false; // Can't currently deal with base-and-index addressing
                            } else {
                                propagationValid = true;
                                // Indicate that instruction uses propagated X29
                                bb.instrs[i].minstr->setUsesPropagatedStackFrame(true);

                                // Replace VarState with stackframe equivalent
                                map<Variable, VarState *> blockDefs = *globalDefs[vi->block->bid];
                                ...
                                VarState *frameVS = getOrInitVarState(frameVar, blockDefs, function);
                                for (auto vp : vi->pred)
                                {
                                ...
                                if (bb.instrs[i].minstr->opcode == ARM64_INS_LDP) {
                                    // In case of a ldp, add an extra stack frame VarState for the second of the 'pair'
                                    frameVar.value += frameVar.size;
                                    frameVS = getOrInitVarState(frameVar, blockDefs, function);
                                    bb.instrs[i].inputs.push_back(frameVS);
                                }

                                bb.instrs[i].inputs.erase(bb.instrs[i].inputs.begin()+inputNo);
                                break; /* Must break out of loop as vector indices will get
                                             messed up after we remove one element */
                            }
                        }
                        inputNo++;
                    }
                    for (auto vo : bb.instrs[i].outputs)
                    {
                        if (vo->type == JVAR_MEMORY && vo->base == vs->value) {
                            if(vo->index != JREG_NULL && vo->index != JREG_INVALID) {
                                propagationValid = false; // Can't currently deal with base-and-index addressing
                            } else {
                                propagationValid = true;
                                ...
                                else // If no definition map has been created for this basic block
                                ...
                                for (auto vp : vo->pred)
                                {
                                    bb.instrs[i].inputs.push_back(vp); // Mem base and offset are still inputs
                                    vp->dependants.insert(&bb.instrs[i]);
                                }
                                bb.instrs[i].outputs.push_back(frameVS);

                                if (bb.instrs[i].minstr->opcode == ARM64_INS_STP) {
                                    // In case of a stp, add an extra stack frame VarState for the second of the 'pair'
                                    frameVar.value += frameVar.size;
                                    frameVS = getOrInitVarState(frameVar, blockDefs, function);
                                    bb.instrs[i].outputs.push_back(frameVS);
                                }

                                bb.instrs[i].outputs.erase(bb.instrs[i].outputs.begin()+outputNo);
                                break; /* Must break out of loop as vector indices will get
                                             messed up after we remove one element */
                            }
                        }
...
propagateX29Instr(Instruction *instr, VarState *vs, int64_t offset, map<BlockID, map<Variable, VarState*> *> globalDefs, Function& function)
{
    ...
    if (instr->opcode == Instruction::Add) {
        ...
        for (auto vi : instr->inputs)
        {
            if (vi->type == JVAR_CONSTANT) {
                ...
                if (vi->value != vs->value) {
                    // A register input other than the preceding vs that we came from
                    isAddImmed = false;
                }
            } else {
        ...
        if (isAddImmed) {
            ...
            instPropagateValid = propagateX29(addDst, offset+immediate, globalDefs, function);
        }
        else {
        ...
        for (auto vi : instr->inputs)
        {
            ...
            if (vi->type == JVAR_MEMORY) {
                if (vs->type == JVAR_REGISTER && vi->base == vs->value) {
                    instPropagateValid = propagateX29(vi, offset, globalDefs, function);
                }
                else {
                    instPropagateValid = false;
                }
        ...
        for (auto vo : instr->outputs)
        {
            ...
            if (vo->type == JVAR_MEMORY) {
                if (vs->type == JVAR_REGISTER && vo->base == vs->value) {
                    instPropagateValid = propagateX29(vo, offset, globalDefs, function);
                }
                else {
                    instPropagateValid = false;
                }
        ...
        instPropagateValid = true; /* Assume everything is fine if a call depends on a register 
