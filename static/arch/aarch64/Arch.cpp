    ...
    for (int i=0; i<63; i++) {
        if (bits & mask) {
            if (i <= 30)
                ...
                regSet.insert(JREG_Q0 + (i - 32)); /* Default assume quad-word use */
        }
        mask = mask << 1;
    }
}
...
{
    v.emplace_back((uint32_t)JREG_X0);
    // ARM don't seem to specify any of X0-X7 specifically for return value, assuming X0
}

enum StackFrameState
{
    LOOK_FOR_FRAME_ALLOC, // Looking for an instruction that allocates a stack frame using preindexed addressing
...
void relift_x29_mem(Function *function)
{
    if(function->hasBasePointer)
    {
        for(auto &instr : function->instrs)
        {
            ...
            {
                if(vs->type == JVAR_MEMORY && vs->base == JREG_X29 && vs->index == JREG_NULL)
...
void analyseStack(Function *function)
{
    ...
    if(function->hasBasePointer)
    {// Iterate through non-terminating basic blocks to check that none of them write to X29,
     // and through the terminating blocks to make sure they all deallocate a correct-sized stack frame
        for(auto &bb : function->blocks)
        {
            if(&bb != entry) // Already checked the start block
            {
                if(function->terminations.find(bb.bid) == function->terminations.end()) // Non-terminating blocks
                {
                ...
                {
                    bool dealloc_found = false; // Ensures we only take into account the first dealloc instr we find
                    bool matching_dealloc_found = false; // True if the first found dealloc instr was the same size as the original alloc
                    for(int i = 0; i < bb.size; i++)
                    {
                        MachineInstruction &instr = *bb.instrs[i].minstr;

                        if((!dealloc_found) && instr.isDeallocatingStackFrame())
                        {
                            dealloc_found = true;
                            if(instr.getStackFrameDeallocSize() == function->stackFrameSize)
                            {
                    ...
                    if(dealloc_found && !matching_dealloc_found) // If a dealloc is found it must match
                    { 
                        function->hasBasePointer = false;
                    }
                }
