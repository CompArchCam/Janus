...
#define PAIR_PREPOST_BIT                                                       \
    23 // The bit that is 1 if using pre or post indexing in STP and LDP
       // 'pairwise' type instructions
#define PAIR_PREINDEX_BIT                                                      \
    24 // The bit that is 1 if preindexing specifically is used in STP and LDP
       // 'pairwise' type instructions
#define LDR_NON_LITERAL_BIT                                                    \
    29 // The bit that distinguishes a literal ldr instruction from  normal ldr
    ...
    : id(id)
{
    ...
    case ARM64_INS_CBNZ:
    case ARM64_INS_CBZ:
        // Swap branch address to first operand (since rest of system is set up
        // to believe the branch address comes first)
        tmp = operands[0];
    ...
    case ARM64_INS_STURH: {
        ...
        raw_instr = ((instr->bytes[3] << 24) | (instr->bytes[2] << 16) |
                     (instr->bytes[1] << 8) | instr->bytes[0]);
    ...
    case ARM64_INS_STP: {
        ...
        raw_instr = ((instr->bytes[3] << 24) | (instr->bytes[2] << 16) |
                     (instr->bytes[1] << 8) | instr->bytes[0]);
        if ((raw_instr & (1 << PAIR_PREPOST_BIT)) &&
            (raw_instr & (1 << PAIR_PREINDEX_BIT))) {
            coarseType |= INSN_PREINDEX;
        } else if ((raw_instr & (1 << PAIR_PREPOST_BIT)) &&
                   !(raw_instr & (1 << PAIR_PREINDEX_BIT))) {
            coarseType |= INSN_POSTINDEX;
        }
    } break;
    ...
    case ARM64_INS_MOVN:
        // movn moves an inverted immediate so convert it to a movz of an
        // immediate
        if (operands[0].size == 4) {
            // For 32 bit values (i.e. going to a 32 bit reg), cast to be
            // careful of signs
    ...
    for (int i = 0; i < nGroup; i++) {
        ...
        case CS_GRP_JUMP:
            ...
            case ARM64_INS_TBZ:
                ...
                break;
            case ARM64_INS_B:
                /* B may or may not be conditional so need a check - if
                   ARM64_CC_INVALID it has no condition, ARM64_CC_AL and
                   ARM64_CC_NV are condition codes that happen to always
                   evaluate to 'true' */
                if (detail->arm64.cc == ARM64_CC_INVALID ||
                    detail->arm64.cc == ARM64_CC_AL ||
                    detail->arm64.cc == ARM64_CC_NV)
                    fineType = INSN_JUMP;
                else
                    fineType = INSN_CJUMP;
                break;
    ...
    for (int i = 0; i < opndCount; i++) {
        if (operands[i].type == OPND_REG) {
            if (opsz < 0)
                ...
                if (operands[i].size !=
                    opsz) { // If 2 different size reg.s, leave as-is
                    opsz = 0;
                }
            }
        }
    ...
    for (int i = 0; i < opndCount; i++) {
        if (operands[i].isMemoryAccess()) {
            ...
            //  but only if this enables a lower bound than the memory
            //  displacement
...
{
    return (opcode == ARM64_INS_CMP || opcode == ARM64_INS_CMN ||
            opcode == ARM64_INS_CCMP || opcode == ARM64_INS_CCMN ||
            opcode == ARM64_INS_FCMP || opcode == ARM64_INS_FCMPE ||
            opcode == ARM64_INS_FCCMP || opcode == ARM64_INS_FCCMPE);
}

bool MachineInstruction::readControlFlag() /* Will have to add cond. instr.
                                              support to this */
{
    return (isType(INSN_CONTROL) && fineType == INSN_CJUMP &&
                (
                    // CJUMP instructions that do not read condition flags (e.g.
                    // CBZ Compare and Branch if Zero - does its own comparing)
                    opcode != ARM64_INS_CBZ && opcode != ARM64_INS_CBNZ &&
                    opcode != ARM64_INS_TBZ && opcode != ARM64_INS_TBNZ) ||
            (
                // Other conditional instructions (that hence read the control
                // flags)
                opcode == ARM64_INS_CCMP || opcode == ARM64_INS_CCMN ||
                opcode == ARM64_INS_CSINC || opcode == ARM64_INS_CSINV ||
                opcode == ARM64_INS_CSNEG || opcode == ARM64_INS_CSEL ||
                opcode == ARM64_INS_CSET || opcode == ARM64_INS_CSETM ||
                opcode == ARM64_INS_CINC || opcode == ARM64_INS_CINV ||
                opcode == ARM64_INS_CNEG || opcode == ARM64_INS_FCCMP ||
                opcode == ARM64_INS_FCCMPE || opcode == ARM64_INS_FCSEL));
}
...
bool MachineInstruction::isAllocatingStackFrame()
{
    ...
    if (isType(INSN_PREINDEX)) {
        for (int i = 0; i < opndCount; i++) {
            if (operands[i].type == OPND_MEM &&
                (operands[i].mem.base == JREG_SP ||
                 operands[i].mem.base == JREG_WSP) &&
...
int MachineInstruction::getStackFrameAllocSize()
{ // User of this function must check that a stack frame is being allocated
  // using isAllocatingStackFrame()
    int frameSize = 0;
    ...
    for (int i = 0; i < opndCount; i++) {
        if (operands[i].type = OPND_MEM) {
            frameSize =
                (-1 *
                 operands[i].mem.disp); // Mem displacement will be negative
                                        // since stack is upside down
        }
    }
    return frameSize;
}

bool MachineInstruction::isSubSP()
{
...
{ // User of this function must check that the instruction is a { sub sp, sp,
  // #immediate } instruction using isSubSP()
    return operands[2].intImm;
}

bool MachineInstruction::isMoveSPToX29()
{
    ...
    for (int i = 0; i < opndCount; i++) {
        if (operands[i].type == OPND_REG) {
            if (operands[i].reg == JREG_X29 &&
                (operands[i].access & OPND_WRITE)) {
                writeX29 = true;
            }
            if ((operands[i].reg == JREG_SP || operands[i].reg == JREG_WSP) &&
                (operands[i].access & OPND_READ)) {
                readSP = true;
            }
        }
...
bool MachineInstruction::isDeallocatingStackFrame()
{
...
int MachineInstruction::getStackFrameDeallocSize()
{ // User of this function must check that a stack frame is being deallocated
  // using isDeallocatingStackFrame()
    int frameSize = 0;

    for (int i = 0; i < opndCount; i++) {
        if (operands[i].type ==
            OPND_IMM) // Assuming the only immediate will be the offset
        {
...
bool MachineInstruction::isX29MemAccess()
{
    for (int i = 0; i < opndCount; i++) {
...
bool MachineInstruction::usesPropagatedStackFrame() { return (aux == 1); }

void MachineInstruction::setUsesPropagatedStackFrame(
    bool value) /* Use aux location to store 1 if uses
