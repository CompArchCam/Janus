...
{
    if (type == OPND_REG) {
        if (reg >= JREG_MM0 && reg < GSEG_ES)
            return true;
        if (reg >= JREG_DR0 && reg < JREG_R8B)
            return true;
        if (reg >= JREG_FP0 && reg < JREG_INVALID2)
            return true;
    }
    return false;
}
...
{
    if (type == OPND_REG) {
        if (reg >= JREG_XMM0 && reg <= JREG_XMM15)
            return true;
        if (reg >= JREG_YMM0 && reg <= JREG_ZMM31)
            return true;
        if (reg >= JREG_XMM16 && reg <= JREG_XMM31)
            return true;
    }
    return false;
}
...

{
    ...
    case OPND_MEM:
        if (structure == STACK_PTR_IMM) {
            ...
            var.value = dispMem.disp +
                        nextInstrPc; // in RIP relative addresses, we need the
