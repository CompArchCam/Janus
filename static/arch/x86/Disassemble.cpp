...
void disassembleAll(JanusContext *jc)
{
    ...
    err =
        cs_open(CS_ARCH_X86, CS_MODE_64, (csh *)(&jc->program.capstoneHandle));
                          ...
                          vector<Variable> &v)
{
    ...
    else if (minstr->opcode == X86_INS_INC || minstr->opcode == X86_INS_DEC) {
        Variable var;
        var.type = JVAR_CONSTANT;
        var.value = 1;
        v.push_back(var);
...
void linkArchSpecSSANodes(Function &function,
