...
static void usage()
{
    ...
    cout << "  -c: generate custom analysis and rules from Cinnamon DSL"
         << endl;
    ...
    cout << "  -pr -noshared: generate rules for automatic loop profiling & "
            "disable shared library profiling"
         << endl;
...
int main(int argc, char **argv)
{
    ...
    if (argv[1][0] == '-') {
        ...
        case 'l':
            if (argv[1][2] == 'c') {
                mode = JLCOV;
                IF_VERBOSE(cout << "Loop coverage profiling mode enabled"
                                << endl);
                break;
            } else {
                usage();
        ...
        case 'f':
            if (argv[1][2] == 'c') {
                mode = JFCOV;
                IF_VERBOSE(cout << "Function coverage profiling mode enabled"
                                << endl);
            } else {
                mode = JFETCH;
        ...
        case 's':
            mode = JSECURE;
            IF_VERBOSE(cout << "Secure execution enabled" << endl);
            break;
        case 'o':
            mode = JOPT;
        ...
        case 'd':
            mode = JDLL;
            IF_VERBOSE(cout << "Testing for dll instrumentation enabled"
                            << endl);
            break;
        default: {
            usage();
