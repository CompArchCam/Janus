...
static void usage()
{
    ...
    cout << "       schedump -h {file}.jrs          : dump rule header only"
         << endl;
    cout << "       schedump -c channel {file}.jrs  : dump rules at specified "
            "channel"
         << endl;
}
...
static void loadRewriteRule(char *name)
{
    ...
    if (!ruleFile.is_open()) {
        cerr << "Janus Rewrite Schedule file " << name << " does not exist"
             << endl;
    }
...
int main(int argc, char **argv)
{
    ...
    if (argc == 2) {
        ...
        if (checkFileFormat(argv[1])) {
            ...
            cerr << "Please supply a rewrite schedule file ending with *.jrs"
                 << endl;
            return 0;
        }
    } else if (argc == 3) {
    ...
    cout << "Rewrite schedule type:  "
         << print_janus_mode((JMode)header->ruleFileType) << endl;
    cout << "Number of rewrite rules: " << header->numRules << endl;
    cout << "Number of loops: " << header->numLoops << endl;
    if (header->multiMode)
        cout << "Multiple loop mode" << endl;
    ...
    if (header->multiMode) {
        ...
        for (int i = 0; i < header->numLoops; i++) {
            cout << "Loop " << loops[i].id << dec << " static id "
                 << loops[i].staticID << " : number of rules " << dec
                 << loops[i].ruleInstSize << " number of data "
                 << loops[i].ruleDataSize << endl;
            RRule *rule = (RRule *)(buffer + loops[i].ruleInstOffset);
            for (int j = 0; j < loops[i].ruleInstSize; j++) {
                print_rule(rule + j);
            }
            JVarProfile *profile =
                (JVarProfile *)(buffer + loops[i].ruleDataOffset);
    ...
    if (mode == 3) {
        ...
        if (header->ruleDataOffset) {
            ...
            if (dataOffset == 1)
                cout << "No data for this loop" << endl;
            else {
    ...
    for (int i = 0; i < header->numRules; i++) {
        if (currentChannel != (int)ruleArray[i].channel) {
            if (header->ruleDataOffset && currentChannel != -1 &&
                currentChannel != 0) {
                /* Get the offset the data in this channel */
                uint32_t *loopArray =
                    (uint32_t *)(buffer + header->ruleDataOffset);
                ...
                if (dataOffset == 1)
                    ...
                    while (dataOffset < dataOffsetEnd) {
                        JVarProfile *profile =
                            (JVarProfile *)(buffer + dataOffset);
                        print_profile(profile);
                        dataOffset += sizeof(JVarProfile);
                    }
                }
            }
            cout << "Channel " << (int)ruleArray[i].channel
                 << ": -----------------------------" << endl;
