...
enum LoopExitType { CONDITIONAL_EXIT, BREAK_EXIT, IRREGULAR_EXIT };
...
class Loop
{
  public:
    ...
    bool pass;
    /** \brief The loop is not beneficial from coverage profiling (low
     * coverage/iteration) */
    bool removed;
    ...
    int level;
    /** \brief Set of loops that are ancestor of this loop in the loop relation
     * graph */
    std::set<Loop *> ancestors;
    /** \brief Set of loops that are descendant(all sub-loop) of this loop in
     * the loop relation graph */
    ...
    /** \brief Collection of variables to be made private - those that are read
       only but must be privatised for some other reason */
    ...
    /** \brief Collection of constant phi variables created by compiler register
     * spilling */
    ...
    RegSet iteratorRegs;
    /** \brief Iteration count from static analysis, if zero, it means the
     * iteration count can't be determined */
    ...
    /** \brief a set of recognised memory locations in the loop, indexed by
     * array bases (common base)
     *
    ...
    /// The set of registers needed to merge after threads finish, but need to
    /// track which thread last modified the register
     ...
     *
     * The encoded variables is directly copied into the rewrite schedule and
     * read by the dynamic component */
    ...
    /** \brief returns whether the value of vs is (assumed to be) the same for
     * every iteration
     *
     * After variableAnalysis(this) has been called, this function returns
     * whether vs holds the same value in every iteration of the loop.
     *
     * Complexity: *O(L)*, where *L = max(this->level, vs->constLoop->level)*,
     *             as an LCA with *vs->constLoop* may be calculated.
     *
     * Note, that false positives are sometimes possible, see
     * VarState::dependsOnMemory.
     */
    bool isConstant(VarState *vs);
    /** \brief returns whether the value of vs is only dependent on the
     * induction variable of the loop
     *
     * It traverses back in the SSA graph. If vs is calculated by vs = foo(i,
     * C1, C2). It is treated as an independent variable
     */
    bool isIndependent(VarState *vs);
    /** \brief returns whether the value of vs is freshly loaded from outside of
     * the loop */
    bool isInitial(VarState *vs);
    /** \brief returns whether v is the main induction variable with check
     * condtions */
     ...
     *
     * Returns the innermost loop A, such that both this and l are nested within
     * A (possibly through multiple levels of nesting).
     *
     * If *this* and *l* have no common ancestor, they are in different
     * functions or l==NULL, then NULL is returned.
     */
    ...
    /** \brief Return a variable state whose the storage is constant and
     * non-immediate.*/
    VarState *getAbsoluteStorage(VarState *vs);
    /** \brief Return the encoded profile for the variable : init, range, check
     * etc. */
...
void linkParentLoops(janus::Function *function);
