...
public:
    ...
    BasicBlock                      *succ1;
    ///Second successor in the CFG (only available for blocks ended with conditional branches)
    BasicBlock                      *succ2;
    ///A fake basic block is a block that does not end with control flow instruction, but ended with an incoming branch target
    bool                            fake;
     ...
     * If you want to modify the instructions, please allocate another chunk of memory
     */
    Instruction                     *instrs;
    /** \brief Array instructions that contain memory accesses in the basic block
     *
    ...
    /** \brief Input variables used in the block without initialization/assignment first */
     ...
     *
     * Set of blocks b, such that this block dominates an immediate predecessor of b, but does not
     * strictly dominate b.
     */
    ...
    Loop                            *parentLoop;
    BasicBlock(Function *func, BlockID bid, InstID startInstID, InstID endInstID, bool isFake);
    ...
    /** \brief Returns the SSA definition (of the given variable) used at the start of the current
     *         block.
     *
     * A traversal on the basic blocks is performed to find the definition.
     */
    VarState                        *alive(Variable var);
    /** \brief Returns the SSA definition (of the given variable) alive at (right before) the  
     *         index'th instruction of the block.
     */
    VarState                        *alive(Variable var, int index);

    /** \brief Is the given register live just before the index'th instruction?
     *
     * Is the current value just before the index'th instruction in this block in the register
     * given ever used later (either within the same block or in possible successor blocks)?
     */
    bool isRegLive(uint32_t reg, int index);

    /** \brief Returns the set of registers live just before the index'th instruction.
     *
     * The i'th entry in the returned *RegSet* is set iff *isRegLive(i,index)* would return true.
     */
    RegSet liveRegSet(int index);

    /** \brief If the given register is assigned just before the index'th instruction, can the new
     *         value reach the calling function?
     *
     * *index* should be between 0 and size **both inclusive**. If index==size, then it considers
     * assigning just after the last instruction.
     */
    bool doesReachOutside(uint32_t reg, int index);

    /** \brief Returns the set of registers for which, if assigned just before the intex'th
     *         instruction, the new value can reach the calling function.
     *
     * The i'th entry in the returned *RegSet* is set iff *doesReachOutside(i,index)* would return
     * true.
     */
