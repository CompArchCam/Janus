 ...
 *
 * - The **Instruction** class should be machine independent. For machine dependent instruction, see the **MachineInstruction** class.
...
public:
    ...
    enum Opcode {
        ///If the opcode is undefined, it means Janus is not interested in its analysis, you can find the detailed opcode from **MachineInstruction**
        Undefined,
        /* Terminator Instructions - These instructions are used to terminate a basic block of the program. */
        DirectBranch,
        ConditionalBranch,
        Call,
        Return,
    ...
    ///Return true if the instruction contains memory accesses (including stack)
    bool                            isMemoryAccess();
