...
/** \brief Represents a memory location/ a close form group of locations in SSA form in a loop */
...
public:
    ...
    enum AddressType {
        ...
        Unknown,
        ///The memory address is a linear function of loop iterators addr = A*i + B (A is non-zero)
        Affine1D,
        ///The memory address is a linear function of multiple loop iterators addr = A*i + B*j + C*k + D (A could be zero)
        AffinemD,
        ///The memory address is a constant (that does not vary between loop iterations)
        Constant,
    ...
    ExpandedExpr                     expr;
    /** \brief Scalar evolution of this memory address, call **getSCEV** to set this value */
    SCEV                             *scev;
    ...
    /** \brief The address of this location is not determined by the induction variable of the loop */
    //bool                             isIndependentOfInduction(Loop *loop);
};

/** \brief The less than operand is only used for sorting memory locations in a set */
bool operator<(const MemoryLocation &a, const MemoryLocation &b);
/** \brief Two memory locations are only equal when its scalar evolution are exactly the same */
bool operator==(const MemoryLocation &a, const MemoryLocation &b);

std::ostream& operator<<(std::ostream& out, const MemoryLocation& mloc);

/** \brief Represents an add recurrences which can be represented as [start, stride]@iter */
class SCEV
{
public:
    enum SCEVType {
...
janus::ExpandedSCEV getRangeDiff(janus::ExpandedSCEV &e1, janus::ExpandedSCEV &e2, janus::Loop *loop);
#endif
