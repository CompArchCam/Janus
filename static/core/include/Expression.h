...
/** \brief An abstract instruction/statement that represents a data flow
 * operation
 *
 */
class Expr
{
  public:
    ...
    mutable ExpandedExpr *expandedCyclicForm;
    /** \brief Link to the expanded form in loop scope */
    mutable ExpandedExpr *expandedLoopForm;
    /** \brief Link to the expanded form in function scope */
    mutable ExpandedExpr *expandedFuncForm;
    /** \brief The expression is an iterator of loop */
    mutable Iterator *iteratorTo;
    union {
    ...
    Expr()
        : kind(NONE), vs(NULL), expandedLoopForm(NULL), expandedFuncForm(NULL),
          expandedCyclicForm(NULL), iteratorTo(NULL)
    {
    }
    Expr(int i)
        : kind(INTEGER), i(i), vs(NULL), expandedLoopForm(NULL),
          expandedFuncForm(NULL), expandedCyclicForm(NULL), iteratorTo(NULL)
    {
    }
    Expr(int64_t i)
        : kind(INTEGER), i(i), vs(NULL), expandedLoopForm(NULL),
    ...
    Expr(ExpandedExpr *ee)
        : kind(EXPANDED), ee(ee), vs(NULL), expandedLoopForm(NULL),
          expandedFuncForm(NULL), expandedCyclicForm(NULL), iteratorTo(NULL)
    {
    }
    ...
    void negate();
    /// Return an ExpandedExpr of type MUL representing a left shift of the
    /// current expr by another expr
...
/** \brief A collapsed representation of a sub-AST that generates the value of a
 * given **Expr**.
 *
 * The **ExpandedExpr** class is used to represent the arithmetic relation from
 * a set of leaf **Expr** in the AST. The leaf **Expr** is defined with a scope,
 * for example, a loop boundary or a function frame. The expansion of the
 * ExpandedExpr will stop if it hits the predefined boundary.
 *
 ...
 * (coeff1*term1 + coeff2*term2 + coeff3*term3), where the term must be
 * different leaves **Expr**
 *
 * An expanded **ExpandedExpr::PHI** expression means the expression can only be
 * one of the terms at a time: \code 'phi' term1*(1), term2*(1), term3*(1)
 * \endcode It refers to an option from (term1, term2, term3), where term could
 * be another **ExpandedExpr** (but none of them would be
 * **ExpandedExpr::PHI**).
 *
 * The **ExpandedExpr** class always maintains its internal order so that there
 * is no other isomorphic form. When the **ExpandedExpr::PHI** and
 * **ExpandedExpr::SUM** expressions are merged, the **ExpandedExpr::PHI** is
 * always place on the top. For example, \code A = (a,b) B = (c,d) A+B =
 * (a+c,a+d,b+c,b+d) \endcode
 */
class ExpandedExpr
{
  public:
    enum ExpandedScope {
        ...
        FunctionScope, /// Expansion stops when reaching outside of the current
                       /// function
    };
     ...
     *
     * the lvalue (const) represents a leaf expression node according to the
     * boundary the rvalue represents the coefficient after expansion
     */
    ...
    /// Return true if the expression has no terms that are related to loop
    /// iterators
    bool isConstant();
    ...
    /// Infer the array base from the current expression, return NULL if no
    /// array base is found
    ...
    /// Return the value of loop's normalised boundary (0,1,2..N) if the
    /// expression is evaluated as zero. Return -1 if failed.
    ...
    /// Expand the leaf nodes of the current expression to memory boundary /
    /// register leaf should be replaced unless function arguments
    void extendToMemoryScope(Function *func, Loop *loop);
    /// Expand the the current expression to the terms that contains the
    /// variable, return false if the variable is not found
 ...
 * \param[in] loop The loop that the expansion would stop if traversal goes
 * beyond the loop \return The pointer to the expanded expression, (also
 * buffered in expr->expandedLoopForm).
 */
janus::ExpandedExpr *expandExpr(/*IN*/ janus::Expr *expr, janus::Loop *loop);
/*!
 * Expand the current expression to canonical form
 * \param[in] expr The expression class to be expanded
 * \param[in] func The function that the expansion would stop if traversal goes
 * beyond the function \param[in] loop The loop that the expression belongs to.
 * The expanded expansion may be different for the same expression but from
 * different loop (inner or outer). \return The pointer to the expanded
 * expression, (also buffered in expr->expandedFuncForm).
 *
 * It is suggested that the expansion happens after the first pass of loop
 * iterator analysis So that the expansion can recognise loop iterator related
 * expressions
 */
...
void buildExpr(janus::Expr &expr, std::set<janus::Expr *> &exprs,
               janus::Instruction *instr);

bool buildLoopExpr(janus::Expr *expr, janus::ExpandedExpr *expanded,
                   janus::Loop *loop);
/* Graph traversal function in the AST tree */
bool buildFuncExpr(janus::Expr *expr, janus::ExpandedExpr *expanded,
                   janus::Function *func, janus::Loop *loop, bool stopAtMemory,
                   janus::Variable *stopAtVariable);
