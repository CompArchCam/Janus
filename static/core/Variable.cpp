...
bool janus::operator<(const Variable &a, const Variable &b)
{
    JVarPack v1, v2;
    v1.var = a;
    v2.var = b;
    // ignore size for non-stack variables (JAN-72)
    if (v1.var.type != JVAR_STACK)
        v1.var.size = 0;
    if (v2.var.type != JVAR_STACK)
        v2.var.size = 0;
    if (v1.data1 < v2.data1)
        return true;
    else if (v1.data1 == v2.data1) {
        if (v1.data2 < v2.data2)
            return true;
    }
    return false;
}
