...
void
RegSet::insert(uint32_t reg)
{
    bits = bits | get_reg_bit_array(reg);
}

void
RegSet::remove(uint32_t reg)
{
    bits = bits & (~get_reg_bit_array(reg));
}
...
bool
RegSet::contains(uint32_t reg)
{
    return (bits & get_reg_bit_array(reg)); 
}

void
RegSet::merge(RegSet set)
{
    bits |= set.bits;
}

void
RegSet::subtract(RegSet &set)
{
    bits &= (~(set.bits));
}

void
RegSet::intersect(RegSet &set)
{
    bits &= set.bits;
}
...
uint32_t
RegSet::getNextLowest(uint32_t minReg) {
    return decodeNextReg(bits, minReg);
}

uint32_t
RegSet::popNextLowest(uint32_t minReg) {
...
void
RegSet::toSTLSet(std::set<uint32_t> &regSet)
{
...
RegSet
janus::operator+(RegSet a, RegSet b)
{
...
void
BitVector::insert(uint32_t bitPos)
{
...
void
BitVector::remove(uint32_t bitPos)
{
    uint32_t block_id = bitPos / 32;
    GASSERT(block_id <= blocks.size(), "bit position out of range");
    blocks[block_id] = blocks[block_id] & (~(1<<(bitPos % 32)));
...
bool
BitVector::contains(uint32_t bitPos)
{
    uint32_t block_id = bitPos / 32;
    GASSERT(block_id <= blocks.size(), "bit position out of range");
...
void
BitVector::intersect(BitVector &bv)
{
    uint32_t blockSize = bv.blocks.size();
    //GASSERT(blockSize == blocks.size() &&
      //      bv.leftover == leftover, "Vector length not match during merge");
...
void
BitVector::toSTLSet(set<uint32_t> &set)
{
    uint32_t index = 0;
...
bool
janus::operator==(BitVector &a, BitVector &b)
{
    if (a.blocks.size() == b.blocks.size() &&
        ...
        if ((a.blocks.size()) && (
            (a.blocks[a.blocks.size()-1] ^ b.blocks[a.blocks.size()-1]) & ((1<<a.leftover)-1)))
...
void ScratchSet::insert(uint32_t reg)
{
    ...
    if(regs.reg0 == reg) return;
    if(!(regs.reg1)) {
        regs.reg1 = reg;
        return;
    }
    if(regs.reg1 == reg) return;
    if(!(regs.reg2)) {
        regs.reg2 = reg;
        return;
    }
    if(regs.reg2 == reg) return;
    if(!(regs.reg3)) {
        regs.reg3 = reg;
        return;
    }
...
{
    out<<get_reg_name(s.regs.reg0)<<" "<<get_reg_name(s.regs.reg1)<<" "<<get_reg_name(s.regs.reg2)<<" "<<get_reg_name(s.regs.reg3);
    return out;
}
