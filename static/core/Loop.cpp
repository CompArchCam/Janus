...
searchLoop(JanusContext *gc, Function *function)
{
    ...
    for (int i=0; i<size; i++) {
         ...
         */
        if (succ1 &&
            bb.firstVisitStep > succ1->firstVisitStep &&
        ...
        if (succ2 &&
            bb.firstVisitStep > succ2->firstVisitStep &&
...
:start(start), parent(parent), parentLoop(NULL), analysed(false), level(-1), unsafe(false), dependenceAvailable(false), mainIterator()
{
    pass = false;
    staticIterCount = 0;
    id = globalLoopID++;
...
void
Loop::fullPrint(void *outputStream)
{
    print(outputStream);
}

bool
Loop::contains(BlockID bid)
{
    if (body.find(bid) != body.end()) return true;
    else return false;
}
...
Loop::lowestCommonAncestor(Loop *l)
{
    ...
    if (isDescendantOf(l)) return l;
    if (isAncestorOf(l)) return this;
    ...
    if (!commonAncestors.size()) return NULL;
...
Loop::isConstant(VarState *vs) {
    ...
    if (!vs->constLoop)
        ...
        //If vs is constant in an ancestor, then it must be constant here as well
        return isDescendantOf(vs->constLoop);
    return false;
}

bool
Loop::isInitial(VarState *vs) {
    //skip immediate value
    if (vs->notUsed) return false;
    if (vs->type == JVAR_CONSTANT) return true;
    //VarState defined outside of the loop is constant (otherwise would go through phi node)
    if (body.find(vs->block->bid) == body.end())
        return true;
    return false;
...
VarState *
Loop::getAbsoluteConstant(VarState *vs)
{
    if (!isConstant(vs)) return NULL;
    if (body.find(vs->block->bid) == body.end())
        return vs;
    if (vs->expr->u.op == Expr::MOV) {
        if (vs->expr->u.e->vs) return getAbsoluteConstant(vs->expr->u.e->vs);
        else return NULL;
    }
    return vs;
}
...
VarState *
Loop::getAbsoluteStorage(VarState *vs)
{
...
void
Loop::printDot(void *outputStream)
{
    ostream &os = *(ostream *)outputStream;
    GASSERT((parent!=NULL && parent->entry!=NULL), "Parent function CFG not found");
    BasicBlock *entry = parent->entry;
    ...
    for (auto bid : body) {
        ...
        os <<"\"";
        if (bb.bid == start->bid)
            os<<",style=\"filled\",shape=box,color=black,fillcolor=gold];"<<endl;
        else {
            int r=0, g=0, b=0;
            toRGB(temperature[bb.instrs->pc], &r, &g, &b);
            os<<",style=\"filled\",shape=box,color=black,fillcolor=\"#"<<hex<<setfill('0')<<setw(2)<<r<<setfill('0')<<setw(2)<<g<<setfill('0')<<setw(2)<<b<<"\"];"<<endl;
        }
    }
    for (auto bid : exit) {
        BasicBlock &bb = entry[bid];
