...
Expr::Expr(VarState *vs) : kind(VAR), vs(vs), v(vs)
{
...
bool janus::operator==(const Expr &expr1, const Expr &expr2)
{
    ...
    if (expr1.kind == Expr::VAR) {
        if ((uint64_t)expr1.v == (uint64_t)expr2.v)
            return true;
        else
            return false;
    }
    if (expr1.kind == Expr::MEM) {
        if ((uint64_t)expr1.m == (uint64_t)expr2.m)
            return true;
        else
            return false;
    }
    if (expr1.kind == Expr::EXPANDED) {
        if ((uint64_t)expr1.ee == (uint64_t)expr2.ee)
            return true;
        else
            return false;
    }
    if (expr1.kind == Expr::UNARY) {
        if (expr1.u.op != expr2.u.op)
            return false;
        if ((uint64_t)expr1.u.e != (uint64_t)expr2.u.e)
            return false;
        else
            return true;
    }
    if (expr1.kind == Expr::BINARY) {
        if (expr1.b.op != expr2.b.op)
            return false;
        if ((uint64_t)expr1.b.e1 != (uint64_t)expr2.b.e1)
            return false;
        if ((uint64_t)expr1.b.e2 != (uint64_t)expr2.b.e2)
            return false;
        return true;
    }
    if (expr1.kind == Expr::PHI) {
        if ((uint64_t)expr1.p.e1 != (uint64_t)expr2.p.e1)
            return false;
        if ((uint64_t)expr1.p.e2 != (uint64_t)expr2.p.e2)
            return false;
        return true;
    }
    return false;
    ...
    if (kind == EXPANDED) {
        if (ee->kind == ExpandedExpr::PHI)
            ...
            ASSERT_NOT_IMPLEMENTED(
                "inserting an phi expression into other type of expression");
        }
    } else {
...
// Creates and returns a new ExpandedExpr representing a left shift by an
// immediate as a multiply
...
bool Expr::isLeaf()
{
    if (kind == INTEGER || kind == NONE || kind == MEM || kind == ADDREC)
        return true;

    if (kind == VAR && v->lastModified == NULL)
        return true;
    if (kind == VAR && v->lastModified->opcode == Instruction::Call)
        return true;
    return false;
}
...
{
    if (kind == VAR && vs->type == JVAR_REGISTER)
        return true;
    return false;
}
...
Expr ExpandedExpr::get(Expr node) { return exprs[node]; }
...
Expr ExpandedExpr::get(int i) { return exprs[Expr(i)]; }

bool ExpandedExpr::isEmpty() { return exprs.size() == 0; }

void ExpandedExpr::remove(Expr node) { exprs.erase(node); }
...
void ExpandedExpr::addTerm(int value) { addTerm(Expr(1), value); }
...
void ExpandedExpr::addTerm(Expr *node) { addTerm(*node); }
...
void ExpandedExpr::addTerm(Expr *node, int coeff) { addTerm(*node, coeff); }

void ExpandedExpr::addTerm(Expr *node, Expr &coeff) { addTerm(*node, coeff); }
...
void ExpandedExpr::merge(ExpandedExpr &expr) { merge(&expr); }
...
void ExpandedExpr::sensiMerge(ExpandedExpr &expr) { sensiMerge(&expr); }
...
void ExpandedExpr::subtract(ExpandedExpr &expr)
{
    if (kind == SUM && expr.kind == SUM) {
        for (auto e : expr.exprs) {
            // convert A * 1 to 1 * A
            if (e.first.kind == Expr::INTEGER &&
                e.second.kind == Expr::INTEGER) {
        ...
        *this = expr;
        ...
        cerr << "ExpandedExpr::subtract error subtract different types of "
                "expanded expression!"
             << endl;
        cerr << *this << " - " << expr << endl;
        asm("int3");
...
pair<Expr::ExprKind, int64_t> ExpandedExpr::evaluate(Loop *loop)
{
    if (exprs.size() == 1) {
        auto term = exprs.begin();
        Expr first = (*term).first;
        Expr second = (*term).second;
        if (first.kind == Expr::INTEGER && second.kind == Expr::INTEGER) {
            return make_pair(Expr::INTEGER, first.i * second.i);
        } else if (first.kind == Expr::VAR && second.kind == Expr::INTEGER &&
                   second.i == 1) {
            return make_pair(Expr::VAR, (int64_t)first.v);
        } else if (first.vs && loop->isConstant(first.vs) &&
                   second.kind == Expr::INTEGER && second.i == 1) {
            return make_pair(Expr::VAR,
                             (int64_t)loop->getAbsoluteConstant(first.vs));
        } else if (first.kind == Expr::PHI && second.kind == Expr::INTEGER &&
                   second.i == 1) {
            return make_pair(Expr::VAR, (int64_t)first.vs);
        } else
            return make_pair(Expr::NONE, -1);
    } else
        return make_pair(Expr::NONE, -1);
}

void ExpandedExpr::simplify()
{
...
ostream &janus::operator<<(ostream &out, const Expr &expr)
{
    ...
    case Expr::EXPANDED:
        ...
        break;
    case Expr::ADDREC:
        out << "{" << *expr.rec.base << ",+," << *expr.rec.stride << "}@L"
            << dec << expr.rec.iter->loop->id;
        break;
    default:
        if (expr.vs) {
            out << expr.vs;
...
ostream &janus::operator<<(ostream &out, const ExpandedExpr &expr)
{
    ...
    if (expr.kind == ExpandedExpr::SUM)
        op = '+';
    else if (expr.kind == ExpandedExpr::PHI)
        op = ',';
    else if (expr.kind == ExpandedExpr::MUL)
        op = '*';
    else
        op = '?';
...
ExpandedExpr janus::operator-(const ExpandedExpr &expr1,
                              const ExpandedExpr &expr2)
{
                              ...
                              const ExpandedExpr &expr2)
{
    ...
    if (expr1.kind != ExpandedExpr::SUM || expr2.kind != ExpandedExpr::SUM)
        return result;
...
void ExpandedExpr::extendToFuncScope(Function *func, Loop *loop)
{
    ...
    for (auto e : refs) {
        ...
        if (!term.isLeaf()) {
            ...
            exprs[e.first] = e.second;
    }
    simplify();
}

void ExpandedExpr::extendToLoopScope(Loop *loop)
{
    map<Expr, Expr> refs = exprs;
    exprs.clear();
...
void ExpandedExpr::extendToMemoryScope(Function *func, Loop *loop)
{
    map<Expr, Expr> refs = exprs;
    exprs.clear();
...
bool ExpandedExpr::extendToVariableScope(Function *func, Loop *loop,
                                         Variable *var)
{
    map<Expr, Expr> refs = exprs;
    exprs.clear();
...
void ExpandedExpr::removeImmediate()
{
    for (auto e = exprs.begin(); e != exprs.end();) {
        Expr node = (*e).first;
        Expr coeff = (*e).second;
...
Iterator *ExpandedExpr::hasIterator(Loop *loop)
{
    for (auto e : exprs) {
        Expr term = e.first;
...
bool ExpandedExpr::isArrayBase(Loop *loop)
{
    if (!exprs.size())
        return false;
    for (auto e : exprs) {
...
ExpandedExpr ExpandedExpr::removeIterator(Loop *loop)
{
    ExpandedExpr result(ExpandedExpr::SUM);
    for (auto e : exprs) {
        Expr term = e.first;
...
bool buildLoopExpr(Expr *expr, ExpandedExpr *expanded, Loop *loop)
{
    ...
    /* Step 2: Checks if the expression node reaches to the definition of loop
     * iterators */
    ...
    case Expr::UNARY:
        if (expr->u.op == Expr::MOV) {
            if (!buildLoopExpr(expr->u.e, expanded, loop))
                return false;
        } else if (expr->u.op == Expr::NEG) {
            ExpandedExpr negExpr(ExpandedExpr::SUM);
            if (!buildLoopExpr(expr->u.e, &negExpr, loop))
                return false;
            negExpr.negate();
    ...
    case Expr::BINARY:
        if (expr->b.op == Expr::ADD) {
            if (!buildLoopExpr(expr->b.e1, expanded, loop))
                return false;
            if (!buildLoopExpr(expr->b.e2, expanded, loop))
                return false;
        } else if (expr->b.op == Expr::SUB) {
            if (!buildLoopExpr(expr->b.e1, expanded, loop))
                return false;
            ExpandedExpr negExpr(ExpandedExpr::SUM);
            if (!buildLoopExpr(expr->b.e2, &negExpr, loop))
                return false;
            negExpr.negate();
            expanded->merge(negExpr);
        ...
        } /* else if (expr->b.op == Expr::MUL && expanded->kind !=
        ExpandedExpr::SENSI) { if (expr->b.e2->kind == Expr::INTEGER) {
        ...
        break;
    case Expr::PHI:
        // if it is a phi expression, firstly check if this phi belongs to any
        // loop's iterator
        if (expr->iteratorTo) {
 ...
 * If the starting point is inside a loop, please also supply the belonging loop
 * pointer */
bool buildFuncExpr(Expr *expr, ExpandedExpr *expanded, Function *func,
                   Loop *loop, bool stopAtMemory, Variable *stopAtVariable)
{
    if (expr == NULL)
        return false;
    ...
    case Expr::MEM:
        ...
        break;
    case Expr::VAR:
        // if it is an expression created outside AST construction, the real
        // expection is wrapped
        if (expr->vs->expr && expr->vs->expr->kind != Expr::VAR) {
            if (!buildFuncExpr(expr->vs->expr, expanded, func, loop,
                               stopAtMemory, stopAtVariable))
                return false;
        } else
            expanded->addTerm(expr);
        break;
    case Expr::UNARY:
        if (expr->u.op == Expr::MOV) {
            if (!buildFuncExpr(expr->u.e, expanded, func, loop, stopAtMemory,
                               stopAtVariable))
                return false;
        } else if (expr->u.op == Expr::NEG) {
            ExpandedExpr negExpr(ExpandedExpr::SUM);
            if (!buildFuncExpr(expr->u.e, &negExpr, func, loop, stopAtMemory,
                               stopAtVariable))
                return false;
            negExpr.negate();
            expanded->merge(negExpr);
    ...
    case Expr::BINARY:
        if (expr->b.op == Expr::ADD) {
            if (!buildFuncExpr(expr->b.e1, expanded, func, loop, stopAtMemory,
                               stopAtVariable))
                return false;
            if (!buildFuncExpr(expr->b.e2, expanded, func, loop, stopAtMemory,
                               stopAtVariable))
                return false;
        } else if (expr->b.op == Expr::SUB) {
            if (!buildFuncExpr(expr->b.e1, expanded, func, loop, stopAtMemory,
                ...
                return false;
            ExpandedExpr negExpr(ExpandedExpr::SUM);
            if (!buildFuncExpr(expr->b.e2, &negExpr, func, loop, stopAtMemory,
                               stopAtVariable))
                return false;
            negExpr.negate();
            expanded->merge(negExpr);
    ...
    case Expr::PHI: {
        ...
        if (expr->iteratorTo) {
            ...
            // if the suspect loop iterator is the descendants of the current
            // loop Use the final value of the iterator
                ...
                if (sit->finalKind == Iterator::INTEGER) {
                    ...
                    for (auto term : sit->finalExprs->exprs) {
                        ...
                        if (t.kind == Expr::INTEGER) {
                            ...
                            buildFuncExpr(&t, expanded, func, loop,
                                          stopAtMemory, stopAtVariable);
                    }
                }
            } else {
    ...
    for (auto e : exprs) {
        ...
        if (term.kind == Expr::INTEGER && e.second.kind == Expr::INTEGER) {
            ...
            if (currentLoop == loop) {
                ...
                ExpandedExpr *temp =
                    term.iteratorTo->initExprs->getArrayBase(loop);
                result->merge(temp);
                delete temp;
            } else if (term.iteratorTo->initKind == Iterator::EXPR) {
                // expand the expressions and get the array base
                term.iteratorTo->initExprs =
                    expandExpr(&term.iteratorTo->initExpr, loop->parent, loop);
                ...
                term.iteratorTo->initExprs = expandExpr(
                    term.iteratorTo->initVar->expr, loop->parent, loop);
                // expand the expressions and get the array base
                if (!term.iteratorTo->initExprs) {
