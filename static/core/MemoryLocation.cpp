    ...
    : expr(ExpandedExpr::SUM), escev(NULL), loop(loop)
{
    ...
    if (hasLoopIterator && !hasSubLoopIterator)
        type = Affine1D;
    if (hasSubLoopIterator)
        type = AffinemD;
    if (!hasLoopIterator && !hasSubLoopIterator)
        type = Constant;
}
...
ExpandedSCEV::ExpandedSCEV(SCEV &scev) : start(0)
{
    if (scev.iter == NULL)
        return;
...
{
    if (scev.iter)
        out << "{" << scev.start << ",+," << scev.stride << "}@L" << dec
            << scev.iter->loop->id;
    else
...
bool MemoryLocation::containIterator(Loop *loop)
{
    if (expr.hasIterator(loop))
        return true;
    if (!escev)
        return false;
...
VarState *MemoryLocation::getIndirectMemory()
{
