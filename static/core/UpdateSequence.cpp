...
int UpdateSequence::compute(BasicBlock *bb, int index, int in, 
    ...
    for (auto p : multiplier) {
        ...
        //For now we add mul times. This is not too bad since it results only in at most as many
        //    instructions as the original update had (i += 100*j is translated into i += k where
        //    k is a constant (k==100j), hence this is only 1 update)
        //Still, there might be some possible IMPROVEMENT here (add more complex computations)

        pair<Operand, int> op;
        if (vs->type() == VARSTATE_MEMORY_OPND) {
            int baseid = evaluate(bb, index, vs->phi[0], bb->parentLoop, 0, gen, varid);
    ...
    if (constupd) {
        new_instr *ni = new new_instr((constupd > 0) ? X86_INS_ADD : X86_INS_SUB,
                                                                            bb->instrs[index].pc);
        ni->push_var_opnd(in);
...
pair<Operand, int> UpdateSequence::computeNtimes(BasicBlock *bb, int index, int64_t N,
                                                            vector<new_instr*> &gen, int &varid) {
    if (multiplier.empty()) {
    ...
    //Otherwise start by loading the constant (immediate) update into a register
