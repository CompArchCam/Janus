...
Instruction::Instruction() { minstr = NULL; }

Instruction::Instruction(MachineInstruction *minstr) : minstr(minstr)
{
    if (minstr) {
        id = minstr->id;
        pc = minstr->pc;
...
bool Instruction::isControlFlow()
{
    return (opcode == Instruction::DirectBranch ||
            ...
            opcode == Instruction::Call || opcode == Instruction::Return ||
            opcode == Instruction::Interupt);
}

bool Instruction::isConditionalJump()
{
    return (opcode == Instruction::ConditionalBranch);
}
int Instruction::getTargetInstID()
{
    if (!minstr)
        return -1;
...
void Instruction::printDot(void *outputStream)
{
...
void Instruction::getInputs(std::vector<Variable> &v)
{
    if (minstr) {
...
bool Instruction::isVectorInstruction()
{
#ifdef JANUS_X86
...
std::ostream &janus::operator<<(std::ostream &out,
                                const MemoryInstruction &minstr)
{
