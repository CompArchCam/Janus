...
static bool
isInsideLoop(Loop *loop, MachineInstruction *instr) {
...
static bool
shouldBeBroadcastRule(Loop *loop, MachineInstruction *instr, set<uint32_t> &reductRegs) {
...
void
insertBroadcastRules(Loop *loop, set<MachineInstruction *> &inputs, set<uint32_t> &reductRegs, set<VECT_RULE *> &rules) {
    for (MachineInstruction *instr: inputs) {
        if (shouldBeBroadcastRule(loop, instr, reductRegs)) {
            rules.insert(new VECT_RULE(instr->block, instr->pc, instr->id, VECT_BROADCAST));
        }
    }
}