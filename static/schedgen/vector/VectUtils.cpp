...
void
deleteMapValues(map<uint32_t, inductRegInfo *> &map) {
...
void
insertRegCheckRule(Loop *loop, uint8_t reg, uint32_t value, set<VECT_RULE *> &rules) {
    ...
    for (auto bid: loop->body) {
        ...
        for (auto initBid: loop->init) {
            ...
            rules.insert(new VECT_REG_CHECK_rule(bb, triggerPC, instr->id, startPC - triggerPC, reg, value));
        }
    }
}

void
insertUnalignedRule(Loop *loop, RegSet &unusedRegs, RegSet &freeRegs, set<VECT_RULE *> &rules) {
    // Assume single basic block in body.
    for (auto bid: loop->body) {
        BasicBlock *bb = &(loop->parent->entry[bid]);
        ...
        for (auto initBid: loop->init) {
            ...
            rules.insert(new VECT_LOOP_PEEL_rule(bb, triggerPC, instr->id, startPC-triggerPC,
                unusedRegs.getNextLowest(JREG_RAX), freeRegs.getNextLowest(),loop->staticIterCount));
        }
    }
}

void
insertForceSSERule(Loop *loop, set<VECT_RULE *> &rules) {
    for (auto initBid: loop->init) {
...
void
removeLiveRegs(Loop *loop, RegSet &freeRegs) {
    for (Loop *parentLoop: loop->ancestors) {
        for(auto const& value: parentLoop->iterators) {
            if (value.second.kind == janus::Iterator::IteratorKind::INDUCTION_IMM) {
...
bool
checkDependingRegs(Loop *loop, map<uint32_t, inductRegInfo *> &inductRegs, set<uint32_t> &reductRegs) {
    ...
    for (auto reg: dependingRegs) {
        ...
        if (reductRegs.find(reg) == reductRegs.end() && inductRegs.find(reg) == inductRegs.end()) {
            LOOPLOGLINE("loop "<<dec<<currentLoopId<<" has non-reduct/induct dependent reg.");
            return false;
        }
    }
    return true;
...
void
updateRulesWithVectorWordSize(set<VECT_RULE *> &rules, uint32_t vectorWordSize) {
    for (auto rule : rules) {
