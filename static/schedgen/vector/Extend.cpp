...
static int getVectorWordSize(MachineInstruction *instr, set<InstOp> &singles,
                             set<InstOp> &doubles)
{
...
static void updateInputs(MachineInstruction *instr,
                         set<MachineInstruction *> &inputs)
{
}

static bool isInductionMemoryAccess(Operand &op,
                                    map<uint32_t, inductRegInfo *> &inductRegs)
{
    return (op.structure == OPND_DISPMEM &&
            inductRegs.find(op.dispMem.base) != inductRegs.end()) ||
           (op.structure == OPND_FULLMEM &&
            (inductRegs.find(op.fullMem.base) != inductRegs.end() ||
             inductRegs.find(op.fullMem.index) != inductRegs.end()));
}

static bool getMemoryInfo(MachineInstruction *instr, uint8_t &reg,
                          uint64_t &stride,
                          map<uint32_t, inductRegInfo *> &inductRegs,
                          Operand op, int32_t &disp)
{
    ...
    if (inductRegs.find(op.fullMem.base) != inductRegs.end()) {
        ...
        if (inductRegs.find(op.fullMem.index) != inductRegs.end()) {
            LOOPLOGLINE("loop "
                        << dec << currentLoopId
                        << " has non-incremental induction memory access.");
        ...
        if (op.fullMem.scale != 1) {
            LOOPLOGLINE("loop " << dec << currentLoopId
                                << " has scaled induction memory access.");
            IF_LOOPLOG(instr->print(&fselectLog));
            return false;
        }
        reg = op.fullMem.base;
...
static bool findFullMemoryRegInit(Loop *loop, MachineInstruction *instr,
                                  uint8_t &reg, uint64_t &stride,
                                  map<uint32_t, inductRegInfo *> &inductRegs,
                                  Operand op, int32_t &disp,
                                  set<VECT_RULE *> &rules)
{
            ...
            if (initVar->lastModified) {
                ...
                if (found) {
                    if (reg == op.fullMem.index) {
                        disp +=
                            (initOther * op.fullMem.scale) + op.fullMem.disp;
                        // If after induction update.
                        if (instr->pc > inductRegs[reg]->modifyingPC) {
                            disp += stride;
                        }
                    } else {
                        disp = initOther + (disp * op.fullMem.scale) +
                               op.fullMem.disp;
                        // If after induction update.
                        if (instr->pc > inductRegs[reg]->modifyingPC) {
                            disp += stride * op.fullMem.scale;
...
static bool getFullMemoryAccessInfo(Loop *loop, MachineInstruction *instr,
                                    uint64_t &stride,
                                    map<uint32_t, inductRegInfo *> &inductRegs,
                                    Operand op, int32_t &disp, bool &found,
                                    set<VECT_RULE *> &rules)
{
    uint8_t reg;
    if (!getMemoryInfo(instr, reg, stride, inductRegs, op, disp))
        return false;
    cout << "Reg: " << getRegNameX86(reg) << endl;
    found = findFullMemoryRegInit(loop, instr, reg, stride, inductRegs, op,
                                  disp, rules);
    if (!found) {
        if (loop->regWrites.contains(reg)) {
            LOOPLOGLINE("loop "
                        << dec << currentLoopId
                        << " has memory access with unknown register value.");
            IF_LOOPLOG(instr->print(&fselectLog));
            return false;
        } else {
            // Insert rule to check that register value is positive to ensure no
            // memory dependence.
...
static bool getMemoryAccessInfo(Loop *loop, MachineInstruction *instr,
                                Operand op, uint64_t &stride,
                                map<uint32_t, inductRegInfo *> &inductRegs,
                                VECT_CONVERT_rule *rule,
                                set<VECT_RULE *> &rules)
{
    ...
    if (op.structure == OPND_DISPMEM) {
        ...
        if (!getFullMemoryAccessInfo(loop, instr, stride, inductRegs, op, disp,
                                     foundInit, rules)) {
            return false;
        }
...
static bool collectExtendRuleData(Loop *loop, MachineInstruction *instr,
                                  uint64_t &stride,
                                  map<uint32_t, inductRegInfo *> &inductRegs,
                                  VECT_CONVERT_rule *rule,
                                  set<VECT_RULE *> &rules)
    ...
    for (int i = 0; i < instr->opndCount; i++) {
        ...
        if (op.type == OPND_MEM) {
            ...
            if (isInductionMemoryAccess(op, inductRegs)) {
                if (!getMemoryAccessInfo(loop, instr, op, stride, inductRegs,
                                         rule, rules))
                    return false;
            } else {
                cout << "not aligned" << endl;
...
static bool insertExtendRule(Loop *loop, BasicBlock *bb,
                             MachineInstruction *instr, uint64_t &stride,
                             set<MachineInstruction *> &inputs,
                             map<uint32_t, inductRegInfo *> &inductRegs,
                             RegSet &freeVectRegs, set<VECT_RULE *> &rules)
{
    VECT_CONVERT_rule *rule =
        new VECT_CONVERT_rule(bb, instr->pc, instr->id, 0, false, stride,
                              freeVectRegs.getNextLowest(GREG_XMM0),
                              CHECK_REQUIRED, 0, loop->staticIterCount);
...
static bool isReductionInductionRead(MachineInstruction *instr,
...
static bool
checkReductionInductionRead(MachineInstruction *instr,
                            set<uint32_t> &reductRegs,
                            map<uint32_t, inductRegInfo *> &inductRegs)
{
    ...
    if (isReductionInductionRead(instr, reductRegs, inductRegs)) {
        for (auto output : instr->outputs) {
            ...
            if (var.field.type != VAR_REG) {
                LOOPLOGLINE("loop " << dec << currentLoopId
                                    << " writes reduction/induction variable "
                                       "to non-register type.");
                IF_LOOPLOG(instr->print(&fselectLog));
                return false;
            }
            if (reductRegs.find(var.getRegister()) == reductRegs.end() &&
                inductRegs.find(var.getRegister()) == inductRegs.end()) {
                LOOPLOGLINE("loop " << dec << currentLoopId
                                    << " write reduction/induction variable to "
                                       "non-reduction/induction register.");
                IF_LOOPLOG(instr->print(&fselectLog));
                return false;
            }
        }
...
static bool
isAcceptedNonVectorInstruction(MachineInstruction *instr,
                               map<uint32_t, inductRegInfo *> &inductRegs,
                               set<uint32_t> &reductRegs)
{
...
static bool checkVectorWordSize(MachineInstruction *instr, set<InstOp> &singles,
                                set<InstOp> &doubles, uint32_t &vectorWordSize)
{
    ...
    if (wordSize > 0) {
        if (!vectorWordSize) {
            ...
            LOOPLOGLINE(
                "loop "
                << dec << currentLoopId
                << " contains both single and double precision instructions.");
            return false;
        }
    }
    return true;
...
static bool extendInstruction(Loop *loop, MachineInstruction *instr,
                              set<InstOp> &singles, set<InstOp> &doubles,
                              ...
                              set<VECT_RULE *> &rules)
{
    if ((instr->opcode == X86_INS_PXOR || instr->opcode == X86_INS_VPXOR) &&
        ...
        LOOPLOGLINE("loop "
                    << dec << currentLoopId
                    << " contains pxor instruction with memory access.");
        IF_LOOPLOG(instr->print(&fselectLog));
        return false;
    }
    if (!checkVectorWordSize(instr, singles, doubles, vectorWordSize))
        return false;
    if (!checkReductionInductionRead(instr, reductRegs, inductRegs))
        return false;
    if (!(instr->opcode == X86_INS_CVTSI2SD ||
          instr->opcode == X86_INS_VCVTSI2SD)) {
        if (!insertExtendRule(loop, bb, instr, stride, inputs, inductRegs,
                              freeVectRegs, rules))
            return false;
    }
    return true;
...
static bool tryExtendInstruction(Loop *loop, MachineInstruction *instr,
                                 set<InstOp> &singles, set<InstOp> &doubles,
                                 BasicBlock *bb,
                                 set<MachineInstruction *> &inputs,
                                 uint64_t &stride, uint32_t &vectorWordSize,
                                 ...
                                 RegSet &freeVectRegs, set<VECT_RULE *> &rules)
{
    if (instr->isXMMInstruction()) {
        if (!extendInstruction(loop, instr, singles, doubles, bb, inputs,
                               stride, vectorWordSize, inductRegs, reductRegs,
                               freeVectRegs, rules))
            return false;
    } else {
        if (!isAcceptedNonVectorInstruction(instr, inductRegs, reductRegs)) {
            LOOPLOGLINE("loop " << dec << currentLoopId
                                << " invalid non-vector instruction found.");
            IF_LOOPLOG(instr->print(&fselectLog));
            return false;
        }
    }
                       ...
                       set<VECT_RULE *> &rules)
{
    ...
    for (int i = 0; i < bb->size; i++) {
        ...
        if (!tryExtendInstruction(loop, instr, singles, doubles, bb, inputs,
                                  stride, vectorWordSize, inductRegs,
                                  reductRegs, freeVectRegs, rules))
            return false;
    }
    if (!vectorWordSize) {
        LOOPLOGLINE("loop " << dec << currentLoopId
    ...
    if (stride != vectorWordSize) {
        LOOPLOGLINE("loop " << dec << currentLoopId
                            << " has non-unit stride for " << vectorWordSize
                            << " byte word size.");
        return false;
    }
