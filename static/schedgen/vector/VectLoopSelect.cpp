...
void setSupportedVectorOpcodes(set<InstOp> &supported_opcodes,
...
static int getVectorWordSize(Instruction &instr, set<InstOp> &singles,
                             set<InstOp> &doubles)
{
...
bool isVectorRuntimeCompatible(Loop &loop, set<InstOp> &supported_opcode,
                               set<InstOp> &singles, set<InstOp> &doubles)
{
    ...
    for (auto bid : loop.body) {
        ...
        for (int i = 0; i < bb.size; i++) {
            ...
            if (instr.minstr->isXMMInstruction()) {
                ...
                if (supported_opcode.find(instr.minstr->opcode) ==
                    supported_opcode.end()) {
                    LOOPLOGLINE("loop "
                                << dec << loop.id
                                << " unsupported opcode: " << bb.instrs[i]);
                    return false;
                }
                ...
                if (wordSize > 0) {
                    if (!loop.vectorWordSize)
                        loop.vectorWordSize = wordSize;
                    else if (loop.vectorWordSize != wordSize) {
                        LOOPLOGLINE("loop "
                                    << dec << loop.id
                                    << " contains different vector word size: "
                                    << bb.instrs[i]);
                        return false;
                    }
                }
...
static bool checkLoopIterator(Iterator &iter)
{
    // currently we are only processing on add iterator
    // if (!(iter.vs && iter.vs->lastModified && iter.vs->lastModified->opcode
    // == Instruction::Add))
    //     return false;
    ...
    if (iter.kind == Iterator::REDUCTION_PLUS)
        return false;

    // the update instruction can be retrieved
    if (!iter.getUpdateInstr())
        return false;
    return true;
}

static bool checkLoopIterators(Loop &loop)
{
    for (auto &iter : loop.iterators) {
        if (!checkLoopIterator(iter.second))
            return false;
    }
    return true;
}

void selectVectorisableLoop(JanusContext *gc, set<Loop *> &selected_loops,
                            set<InstOp> &supported_opcode, set<InstOp> &singles,
                            set<InstOp> &doubles)
{
    for (auto &loop : gc->loops) {
        ...
        if (loop.staticIterCount < 4) {
            LOOPLOGLINE("loop " << dec << loop.id << " static iter count:"
                                << loop.staticIterCount);
            continue;
        }
        // Condition 6: check dependencies
        if (loop.memoryDependences.size() > 0 && checkLoopDependencies(loop)) {
            LOOPLOGLINE("loop " << dec << loop.id
                                << " has cross iteration memory dependencies");
            continue;
        }
        ...
        if (!isVectorRuntimeCompatible(loop, supported_opcode, singles,
                                       doubles)) {
            LOOPLOGLINE("loop " << dec << loop.id
                                << " not currently handled in Janus runtime.");
            continue;
        }
        ...
        if (!checkLoopIterators(loop)) {
            LOOPLOGLINE("loop "
                        << dec << loop.id
                        << " induction variable support not yet implemented.");
            continue;
        }
        ...
        if (!checkStrideAlignment(loop)) {
            LOOPLOGLINE("loop " << dec << loop.id
                                << " memory stride not uniform, transformation "
                                   "not yet implemented.");
            continue;
        }
...
void printVectorisableLoops(set<Loop *> &selected_loops)
{
...
static bool checkCompatibleImplementation(Loop &loop)
{
    for (auto read : loop.memoryReads) {
        if (read->type == MemoryLocation::AffinemD)
            return false;
    }
    for (auto write : loop.memoryWrites) {
        if (write->type == MemoryLocation::AffinemD)
            return false;
    }
    return true;
}