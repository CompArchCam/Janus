...
static bool 
checkMemoryDependencies(Loop *loop, uint64_t stride) { 
        ...
        for (auto mem: dep.second) {
            ...
            if (eval.first == Expr::INTEGER) {
                ...
                if (diffImm != 0 &&
                    ...
                    cout << "Double stride: " << (stride * maxUnrollFactor * 2) << endl;
                    bool readAfterWrite = false;
                    for (auto write : dep.first->writeFrom) {
                        for (auto mem : dep.second) {
                            for (auto read : mem->readBy) {
                                if (read->pc >= write->pc) readAfterWrite = true;
                            }
                        }
                    }
                    // Accept read before write dependencies.
                    // and accept other memory dependencies that are separated by at least 2 x maxVectorSize. (2 for safety)
                    if ((diffImm < 0 && !readAfterWrite) || abs(diffImm) >= (stride * maxUnrollFactor * 2)) {
                        continue;
                    }
                }
            }
            LOOPLOGLINE("loop "<<dec<<loop->id <<" memory dependence within vector size*2.");
            return false;
        } 
    }
    return true;
...
static bool
doesMemoryAccessOverlapNaive(Loop * loop, uint64_t stride) {
...
bool checkMemory(Loop * loop, uint64_t stride) {
    ...
    if (!checkMemoryDependencies(loop, stride)) return false;
    return true;
}