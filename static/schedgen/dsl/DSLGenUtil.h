...
void writeToFile(fstream &file, uintptr_t val) { file << val << endl; }
void writeToFile(fstream &file, std::string val) { file << val << endl; }
void writeToFile(fstream &file, char *val) { file << val << endl; }

std::string get_target_name(Instruction &instr)
{
    std::string trgname;
...
int get_num_loops(JanusContext &jc) { return jc.loops.size(); }
