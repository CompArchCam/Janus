...
static uint32_t compileRewriteRuleDataToFile(JanusContext *gc);
...
void generateRules(JanusContext *gc)
{
    ...
    GSTEP("Rewrite schedule file: " << gc->name << ".jrs generated, " << size
                                    << " bytes " << endl);
}
...
void RuleCluster::insert(RewriteRule &rule)
{
    PCAddress addr = rule.blockAddr;
    ruleMap[addr].insert(rule);
}

void RuleCluster::replace(RewriteRule &rule)
{
    PCAddress addr = rule.blockAddr;
...
void RuleCluster::remove(RewriteRule &rule)
{
    PCAddress addr = rule.blockAddr;
    auto &ruleSet = ruleMap[addr];
...
RewriteRule::RewriteRule(RuleOp op, PCAddress blockAddr, PCAddress ruleAddr,
                         uint32_t instr_id)
    ...
    : opcode(op)
{
    ...
    if (pre_insert) {
        ...
        // for post insert, if the instruction is a jump, then insert before the
        // jump
        else {
...
RRule RewriteRule::toRRule(uint32_t channel)
{
    RRule rule;
...
void RewriteRule::print(void *outputStream)
{
    ostream &os = *((ostream *)outputStream);

    os << "HINT - " << opcode << " Block=" << blockAddr << ", Addr=" << ruleAddr
       << "(ID: " << ((uint16_t)ureg0.down) << "), r0=" << reg0
       << ", r1=" << reg1 << endl;
}
...
bool janus::operator==(const RewriteRule &lhs, const RewriteRule &rhs)
{
    return (lhs.id == rhs.id) && (lhs.opcode == rhs.opcode) &&
           (lhs.ruleAddr == rhs.ruleAddr) && (lhs.reg0 == rhs.reg0) &&
           (lhs.reg1 == rhs.reg1);
}
