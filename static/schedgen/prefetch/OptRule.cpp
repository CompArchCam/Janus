...
"Too many operands (>4)!"};
static int PREFETCH_ERROR_COUNTERS[PREFETCH_ERROR_COUNT] = {
0,0,0,0,0,0,0
};
...
template <typename X, typename A>
inline void Assert(A assertion, const char *s)
{
    if( !assertion ) {
        cerr << "ERROR: " << s << endl;
        throw X();
...
//Updates the outer limit given an additional limit (cLimit) and the current loop (cLoop)
inline static void updateOuterLimit(Loop *&outerLimit, Loop *cLimit, Loop *cLoop) {
    ...
    if (!outerLimit) {
        ...
        if (outerLimit->isAncestorOf(cLimit)) outerLimit = cLimit;
    }
}
...
Loop *findPrefetch(VariableState *vs, map<VariableState*,Loop*> &store) {
    ...
    if (vs->type() == VARSTATE_MEMORY_OPND) {
        ...
        //otherwise its prefetch is the same as the prefetch of its base register
        store[vs] = findPrefetch(vs->phi[0], store);

        //If the prefetch would happen in a block not connected to this one, then the value is constant here
        if (!vs->block->parentLoop->isDescendantOf(store[vs])) store[vs] = NULL;

        //for base-index form, check if base is constant -- IMPROVEMENT: if not compute base if possible
        if ((vs->phi.size() > 1) && (store[vs]) && (!store[vs]->isConstant(vs->phi[1]))) store[vs] = NULL;
    ...
    Loop *outerLimit = NULL; //we have to choose a loop nested inside outerLimit
    Loop *loop = NULL;

    for (auto *vsi : mi->inputs) {
        if (!findPrefetch(vsi, store)) {
            //vsi does not depend on an induction variable. We must prefetch in a loop where
            //    it is constant
            if (vsi->constLoop) {
                updateOuterLimit(outerLimit, vsi->constLoop->parentLoop, mi->block->parentLoop);
            } else updateOuterLimit(outerLimit, vsi->block->parentLoop, mi->block->parentLoop);
            continue;
        }
        Loop *l = store[vsi];

        //if it depends on an induction variable of a loop with no (ancestor) relation to 
        //parentLoop, then that value comes from the last iteration and hence it is constant iff 
        //we choose a loop within the LCA loop, otherwise it is a non-induction non-constant 
        //variable. Hence we should count it as a NULL, but note that we have to choose some loop
    ...
    //And *outerLimit* points to a loop, such that we can precompute values only in loops
    //    strictly nested inside it (everything is strictly nested inside NULL)
    //Hence we can prefetch iff *loop* is not NULL and it is strictly nested inside *outerLimit*
...
/** Returns the length of (number of prefetchable loads on) the longest path from the given load
 */
static int countDependantLoads(VariableState *vs, map<VariableState*,Loop*> &prefLoads, 
                                                    Loop *loop, map<VariableState*,int> &store) {
    if (!loop->contains(vs->block->bid)) return 0;

    if (store.find(vs) != store.end()) return store[vs];
    store[vs] = 0;

    for (auto *mi : vs->dependants) {
        for (auto *vso : mi->outputs) {
            store[vs] = max(store[vs], countDependantLoads(vso,prefLoads,loop,store));
        }
    }

    for (auto *vso : vs->linksToPhi) {
        if (vso->type() != VARSTATE_MEMORY_OPND) continue;
        store[vs] = max(store[vs], countDependantLoads(vso,prefLoads,loop,store));
    }

    if (prefLoads.find(vs) != prefLoads.end()) store[vs]++;

    return store[vs];
}

inline static int offset(VariableState *vs, int t, map<VariableState*,int> &nDL) {
    return (((double)(nDL[vs]))/t) * PREFETCH_CONSTANT;
}

//Copy to a new variable if it is possibly needed later, otherwise simply define
...
//puts computation in gen, which puts the requested induction variable into the variable ID that is returned
static int getInductionVar(BasicBlock* bb, int index, VariableState* vs,
    ...
    for (auto *v : vs->inductionLoop->inductionVars) {
        if (bb->alive(v->getVar(),index) != v) continue;
        if (v->repInductionVar == vs->repInductionVar) {
            ...
            //Calculate vs from v
            UpdateSequence us = (*vs->constDifference)-(*v->constDifference);//update from v to vs
            return us.compute(bb, index, defvarid, gen, varid);
           
        }
    }

    assert("No matching induction variable found" == "which should never happen");
}

//Puts computation in gen, which stores the value of the requested variable in offset number of iterations in the future into a variable, and returns the ID of the result
static int getFutureInductionVar(BasicBlock* bb, int index, VariableState* vs, int offset, 
                                                            vector<new_instr*> &gen, int &varid) {
    //compute current value
    int current = getInductionVar(bb,index,vs,gen,varid);

    pair<Operand, int> upd = vs->update->computeNtimes(bb, index, offset, gen, varid);

    //now update contains the sum of the updates over the next offset iterations. Add it to the 
    //current value to obtain future value
    new_instr* ni = new new_instr(X86_INS_ADD, bb->instrs[index].pc);
    ni->push_var_opnd(current);
    ni->push_opnd(upd.second, upd.first);
    if (upd.second >= 0) ni->inputs.push_back(upd.second);
...
//evaluate the state vs, offset iterations into the future and insert computation at the given 
//                                                                                        position
//vs must solely depend on induction variables and constants in loop and not cross any phi nodes
//the variable ID of the result is returned
int evaluate(BasicBlock* bb, int index, VariableState *vs, Loop *loop, int offset, 
                           vector<new_instr*> &gen, int &varid) {
    if (vs->isConstant) {
        return -1;
        ...
        if (v)
            return copyIfNeeded(bb, index, v, gen, varid); //equivalent variable found, return
        ...
        if (vs->type() == VARSTATE_PHI) {
            for (auto *v : vs->phi) {
                if (loop->body.find(v->block->bid) == loop->body.end()) continue;
                if (v->type() == VARSTATE_PHI) continue;
    ...
    if (vs->type() == VARSTATE_MEMORY_OPND) {
        //IMPROVEMENT: possibly optimise the case where the update is constant by making use of
        //LEA or just [base+offset] form arguments (by putting the constant update as an
        //immediate value in offset rather than generate a computation for it).
        ...
        Assert<PrefetchFailed>(vs->type() == VARSTATE_NORMAL, PREFETCH_ERROR_PHI_NODE_CROSSED);
        ...
        if (mi->opcode == X86_INS_LEA) {
            //for LEA, the input is linked as the base register, rather than a memory operand,
            //since this represents logically correctly what the instruction does.
            //here we need to handle it separately.
            //LEA always has two operands in X86, the first one being the output

            VariableState *vsi = NULL;
            Variable var((uint32_t)mi->operands[1].compress(mi->pc).getRegister());
            for (auto *vss : mi->inputs)
                 if (vss->getVar() == var) vsi = vss;
            ...
            //Register copying -- we omit this, since we will have our own register allocations
            return evaluate(bb, index, mi->inputs[0], loop, offset, gen, varid);
        } else if (ni->fixedRegisters) {
            //fixed register locations -- include the location of all inputs and outputs
            map<Variable,int> vars;
            for (auto *v : mi->inputs) {
                //compute inputs
                if ((v->type() == VARSTATE_STK_VAR) || (v->type() == VARSTATE_RIP_VAR)) {
            ...
            for (auto *v : mi->outputs) {
                if (v->getVar().isEFLAGS()) continue; //don't count EFLAGS register
            ...
            for (int i=0; i<mi->opndCount; i++) {
                Assert<PrefetchFailed>(vars.find(mi->operands[i].compress(mi->pc)) != vars.end(),
                                       PREFETCH_ERROR_FIXED_OPND_NOT_FOUND);
                ni->push_opnd(vars[mi->operands[i].compress(mi->pc)], mi->operands[i]);
            }
            ...
            for (int i=0; i<mi->opndCount; i++) {
                ...
                if (mi->operands[i].access & OPND_READ) {
                    ...
                    for (auto *vv : mi->inputs) {
                        if (vv->getVar() == mi->operands[i].compress(bb->instrs[index].pc))
                            v = vv;
                    }
                ...
                if (mi->operands[i].access & OPND_WRITE) {
                    if (vs->getVar() == mi->operands[i].compress(bb->instrs[index].pc)) {
...
void markLive(uint64_t reg, int index /*of the starting prefetch sequence*/, BasicBlock *bb,
                        vector<RegSet> &registersAvailable, multimap<BasicBlock*,int> &prefInd,
                         vector<pair<BasicBlock*,uint64_t> > &pos, pair<BasicBlock*,int> defPos,
                        set<BasicBlock*> &reached) {
    if (reached.find(bb) != reached.end()) return;
    reached.insert(bb);
    int lim(-1);
    if (bb==defPos.first) lim = defPos.second;

    //remove reg from all prefetch sequences in this block (before seq. with index)
    auto pit = prefInd.equal_range(bb);
    for (auto it = pit.first; it != pit.second; it++) {
        if ((bb == pos[index].first) && (pos[it->second].second >= pos[index].second)) continue;
        if (pos[it->second].second <= lim) continue;

        registersAvailable[it->second].remove(reg);
    }

    if (defPos.first == bb) return;
    //Otherwise recurse
    for (auto *pb : bb->pred) {
        markLive(reg, index, pb, registersAvailable, prefInd, pos, defPos, reached);
    }
}
...
/** \brief Struct to hold all necessary information for deciding the variable-register assignment.
 *
 ...
 * savable    : a queue of registers that are live, but not used for abstract variables, hence
                they are not in constant use, therefore they could be saved (elements near the 
                front are considered to be easier to be saved)
 * slot_used  : bool array of size SPILL_COUNT marking which spill slot is currently being used
 * reg_save   : if the original register A is saved to register/slot B then (reg_save[A]==B)
 * saved_content : (reg_save[A] == B) iff (saved_content[B] == A)
 * assigned   : the A'th entry stores the register that currently contains the A'th abstract var
 ...
 * relocatedOriginal : set of registers that were live through the point where the prefetch code
 *                     is inserted and are at a different place at any instruction that could 
 *                     fault (or at the end)
 * liveRegs   : registers that need to be the same before and after the prefetch code
 * unusedRegs : registers and spill slots that are never used during this prefetch sequence
 ...
 * nonRegVar  : if a variable is defined as a non-register location, the association is stored here
 */
struct AssignmentContext {
    ...
    AssignmentContext(int N, int M): N(N),M(M) {
        ...
        for (int i=1; i<=SPILL_COUNT; i++) unusedSpace.insert(-i);
    }

    void removeFreeUp(int64_t reg) {
        if (assignment.find(reg) == assignment.end()) return;
...
inline static void markAsOverwritten(AssignmentContext &ac, int64_t reg) {
    ...
    if (ac.saved_content.find(reg) == ac.saved_content.end()) return;
    ...
    if (src < 0) {
        ...
        Assert<PrefetchFailed>(dst >= 0, PREFETCH_ERROR_SPILL_TO_SPILL);
            //Spill slot to register
                ...
                cerr << "Rule: restore register " << getRegNameX86(dst)
                     << " from slot " << -src << endl;
            #endif
    } else {
        ...
        if (dst < 0) {
            ...
            #ifdef DEBUG_ASSIGNMENTS
                cerr << "Rule: Save register " << getRegNameX86(src)
                     << " to slot " << -dst << endl;
            #endif
        } else {
            //Register to register
            #ifdef DEBUG_ASSIGNMENTS
                cerr << "Rule: MOV " << getRegNameX86(dst) << ", " << getRegNameX86(src) << endl;
            #endif
        }
    }

    RewriteRule *rule = OPT_INSERT_MOV_rule(ac.bb, ac.pc, ac.rule_ID++, dst, src).encode();
    ac.rules.push_back(rule);
}

//IMPROVEMENT! the save mechanism could be more efficient, only saving if the instruction sequence could actually fault, and restore registers manually in the end. Also, it could just let live registers not read during the sequence be overwritten if they are saved&restored anyway
static void save_original(int64_t reg, AssignmentContext &ac) {
    cerr << "Rule: Register " << getRegNameX86(reg) << " should be saved in the previous prefetch"
                                                    << " sequence to ";
    int64_t r(0);
    if (ac.unusedSpace.size()) {
        //take the last element (prefer registers over spill slots)
        auto it = ac.unusedSpace.end(); it--;
        r = *it;
        assert(r);
        ac.unusedSpace.erase(it);
        if (r < 0) {
            cerr << "spill slot " << -r << endl;
        } else cerr << "register " << getRegNameX86(r) << endl;
    } else {
        cerr << "the stack" << endl;
    }

    RewriteRule *rule = OPT_RESTORE_REG_rule(ac.bb, ac.pc, ac.rrule_ID++, (uint32_t)reg, r)
                                                                                      .encode();
    ac.rules.push_back(rule);
}

//perform a cyclic shift on ac.savable to bring to the front an element that is not used in the next instruction. Returns whether it was successful or not.
//IMPROVEMENT: complexity may become quadratic which may lead to suboptimal performance
inline static bool shift_savable(AssignmentContext &ac) {
    queue<uint32_t> Q;

    while ((!ac.savable.empty()) && (ac.usedNext.find(ac.savable.front()) != ac.usedNext.end())) {
...
inline static int64_t find_free_space(int e, AssignmentContext &ac) {
    ...
    //Try to save a variable (Assumption: any savable variable is unlikely to be in constant use)
    if (shift_savable(ac)) {
...
inline static int8_t correctSize(int8_t reg, Operand& opnd) {
    if ((opnd.type == OPND_REG) && (opnd.size == 4)) return reg + JREG_EAX - JREG_RAX;
    return reg;
}
...
inline static int32_t getRegOfSize(int8_t reg, int size) {
    ...
    switch (size) {
        case 8: return reg;
        case 4: return reg - JREG_RAX + JREG_EAX;
        case 2: return reg - JREG_RAX + JREG_AX;
        case 1:
            if (reg < 4) return reg - JREG_RAX + JREG_AL;
            if (reg < 8) return reg - JREG_RSP + JREG_SPL;
...
static void other_instr(new_instr *ni, AssignmentContext &ac) {
    for (auto &p : ni->opnds) {
        if (p.second == -1) continue; //immediate operand
        if (ac.nonRegVar.find(p.second) != ac.nonRegVar.end()) continue; //non-register opnd
        p.first.setRegister(ac.assigned[p.second]);
    }

    if (ni->hasMemoryAccess()) {
        //Instruction could fault, we need to push to the stack all live registers currently 
        //relocated from their original register
        for (auto p : ac.reg_save) {
            if (p.first == p.second) continue; //not relocated
            if (ac.liveRegs.find(p.first) == ac.liveRegs.end()) continue; //not live
            ac.relocatedOriginal.insert(p.first);
        }
    }

    if (ni->type() == NEWINSTR_DEF) return;
    ...
    for (int i=0; i<ni->opnds.size(); i++) {
        if (ni->opnds[i].first.type != OPND_REG) continue; //not a register
    ...
    if (ni->cloned) {
        ...
        Assert<PrefetchFailed>(ni->opnds.size() <= 4, PREFETCH_ERROR_OPND_COUNT);
        uint8_t opnds[4];
        int opc(0);
        //XXX this ordering may be incorrect if there are multiple input & output operands -- change encoding in the future to match operands 
        for (int i=ni->opnds.size()-1; i>=0; i--) {
            bool isInput(0);
            for (auto x : ni->inputs)
                isInput |= (ni->opnds[i].second == x);
            if (isInput) opnds[opc++] = correctSize(ac.assigned[ni->opnds[i].second],
        ...
        for (int i=ni->opnds.size()-1; i>=0; i--) {
            ...
            for (auto x : ni->outputs)
                isOutput |= (ni->opnds[i].second == x);
            if (isOutput) opnds[opc++] = correctSize(ac.assigned[ni->opnds[i].second],
                                                                        ni->cloned->operands[i]);
        }

        rule = OPT_CLONE_INSTR_rule(ni->cloned->block, ni->cloned->pc, ac.rule_ID++, opnds,
                                                     ni->type()==NEWINSTR_FIXED, ac.pc).encode();
    } else if (ni->opcode == X86_INS_PREFETCH) {
        //PREFETCH instruction
        assert(ni->opnds.size() == 1);

        rule = OPT_PREFETCH_rule(ac.bb, ac.pc, ac.rule_ID++, ni->opnds[0].first.compress(ac.pc))
                                                                                      .encode();
        ...
        rule = OPT_INSERT_MEM_MOV_rule(ac.bb, ac.pc, ac.rule_ID++,
                        getRegOfSize(ni->opnds[0].first.compress(ac.pc).getRegister(),
                                                                       ni->opnds[1].first.size),
                                                  ni->opnds[1].first.compress(ac.pc)).encode();
        ...
        if ((ni->opnds[1].first.type == OPND_IMM)
            ...
            ac.rules.push_back(
                OPT_INSERT_MEM_MOV_rule(ac.bb, ac.pc, ac.rule_ID++,
                         ((uint32_t)reg), ni->opnds[1].first.compress(ac.pc)).encode());
        ...
        rule = OPT_INSERT_ADD_rule(ac.bb, ac.pc, ac.rule_ID++, src_opnd_type,
                                   ((uint32_t)ni->opnds[0].first.compress(ac.pc).getRegister()),
                                                            ni->opnds[1].first.value).encode();
        ...
        bool src_opnd_type = ni->opnds[1].first.type == OPND_REG;
        rule = OPT_INSERT_SUB_rule(ac.bb, ac.pc, ac.rule_ID++, src_opnd_type,
                                   ((uint32_t)ni->opnds[0].first.compress(ac.pc).getRegister()),
                                                            ni->opnds[1].first.value).encode();
    } else {
        //IMUL instruction
        ...
        rule = OPT_INSERT_IMUL_rule(ac.bb, ac.pc, ac.rule_ID++,
                                   ((uint32_t)ni->opnds[0].first.compress(ac.pc).getRegister()),
                                                            ni->opnds[1].first.value).encode();
    }
...
//Returns a pointer to the only VariableState in vs->phi, outside of the given loop if it exists
//NULL otherwise
static VariableState *getOnlyInitialization(VariableState *vs, Loop *loop) {
    ...
    for (auto *v : vs->phi) {
        if (loop->contains(v->block->bid)) continue;
        outside_cnt++;
        ret = v;
    }
...
void
generateOptRules(JanusContext *gc)
{
    RewriteRule rule;
    ...
    for (auto &function : gc->functions) {
        ...
        for (auto &bb : function.mblocks) {
            if (!bb.parentLoop) continue; //skip blocks not within any loops
            for (int i=0; i<bb.size; i++) {
                auto &mi = bb.instrs[i];
                if (!mi.hasMemoryAccess()) continue;

                for (VariableState *op : mi.inputs) {
                    if (!op->block->parentLoop) continue;
                    if (op->type() != VARSTATE_MEMORY_OPND) continue;
                    //op contains a load instruction which is in a loop

                    Loop *loop = NULL;
                    if ((op->phi.size()) && (op->phi[0]->type() == VARSTATE_PHI)
                            && (!op->phi[0]->inductionLoop)
                            && (getOnlyInitialization(op->phi[0], op->block->parentLoop))) {
                        VariableState *vs = getOnlyInitialization(op->phi[0],
                                                                    op->block->parentLoop);
                        loop = findPrefetch(vs, store);

                        //for base-index form, check if base is constant
                        //                          IMPROVEMENT: if not compute base if possible
                        if ((vs->phi.size() > 1) && (loop) && (!loop->isConstant(vs->phi[1])))
                                                                         store[vs] = loop = NULL;
                        phiJump.insert(op);
                        if (initPrefetchLoops.find(loop) != initPrefetchLoops.end()) continue;
                        initPrefetchLoops.insert(loop);
                    } else loop = findPrefetch(op, store);

                    if (!loop) continue; //no prefetch
        ...
        for (auto p : prefetchedLoads) {
            cerr << "Prefetchable load instruction found in loop " << p.second->id << ": "
                 << endl;
        ...
        //Number of loads depending on this state - only meaningful for load instructions
        ...
        for (auto p : prefetchedLoads) {
            countDependantLoads(p.first, prefetchedLoads, p.second, nDL[p.second]);
        ...
        for (auto p : prefetchedLoads) {
            ...
            if (phiJump.find(p.first) != phiJump.end()) { // if we crossed a phi node
                toEvaluate = getOnlyInitialization(p.first->phi[0], p.first->block->parentLoop);

                if (toEvaluate->type() == VARSTATE_NORMAL) {
                    //if it is a normal variable, then after the definition
                    mi = toEvaluate->lastModified;
                    mi++;
                } else mi = toEvaluate->block->instrs;
                       //otherwise to the start of its block (eg phi node)
            } else mi = p.first->dependants[0];

            int offs = offset(p.first, offsetT[p.second], nDL[p.second]);
            //generate computation for the base address, store variable ID in "result"
            int result = evaluate(mi->block, mi->id - mi->block->instrs[0].id, toEvaluate,
                                                            p.second, offs, gen, varid);
            ...
            cerr << endl << "Prefetch generated at address " << mi->pc << " (offset=" << offs << "): " << endl;
            ...
            //Registers originally available (as if this were the only set of new instructions 
            //                                                                          inserted)
            //This is refined later
            registersAvailable.emplace_back(mi->block
                                          ->liveRegSet(mi->id - mi->block->instrs[0].id).bits);
            ...
            N++;
                ...
                //      - in some cases if only a part of a register is used (eg ax, ah, r8w)
                //      - if the alternative computation of a phi-node constant fails in evaluate
                cerr << endl << "No prefetch could be generated..." << endl;

                //IMPROVEMENT: retry at a different address or find an optimal address beforehand
            }
        }
        ...
        for (int i=0; i<N; i++) {
            ...
            for (auto reg : readSet) {
                if (!registersAvailable[i].contains(reg)) continue;
                    //proceed only if not marked live

                //Mark live in all prefetch code between this one and the instruction defining
                //     the variable
                VariableState *vs = pos[i].first->alive(Variable((uint32_t)reg));
                int vsindex = (vs->type() == VARSTATE_PHI) ? -1 :
                                                   vs->lastModified->id - vs->block->instrs[0].id;
                set<BasicBlock*> reached;
                markLive(reg, i, pos[i].first, registersAvailable, prefetchIndices, pos,
                                                            {vs->block, vsindex}, reached);
            }
        }
        ...
        try {
            ...
            for (auto e : ac.available)
                                                if (e <= 16) ///////////XXX At the moment we are only using the first 16 general purpose registers, due to failure to encode other mov (movq, ...) instructions in the dynamic part. This limits the number of registers, hence it would be nice to solve (to enable the usage of XMM, ... registers as save slots -> IMPROVEMENT
            ...
            RS.toSTLSet(savable);
            for (auto q : savable) if (q <= 16) ac.savable.push(q);
            savable.clear();
            ...
            for (auto *ni : gens[i]) {
                if (ni->type() != NEWINSTR_DEF) continue;
                for (auto p : ni->def) {
                    int reg = p.second.getRegister();
                    if (reg <= 16) {
                        ac.lastReadOrig[reg] = max(ac.lastReadOrig[reg], ac.lastRead[p.first]);
                    }
                }
            }
            ...
            for (int k=0; k<ac.M; k++) {
                if (ac.lastRead[k] < 0) ac.lastRead[k] = ac.definedAt[k]+1;
            }

            //schedule original registers that are not live for freeUp
            for (int k=0; k<ac.M; k++) {
                if (ac.lastReadOrig[k] <= N) continue;
                if (!registersAvailable[i].contains(k)) continue;
            ...
            for (auto *ni : gens[i]) {
                ...
                if (ni->type() == NEWINSTR_DEF) {
                    for (auto p : ni->def) {
                      //XXX multiple (read-only) variables assigned to the same original register?
                        if (p.second.field.type != VAR_REG) {
                            ac.nonRegVar[p.first] = p.second;
                            continue;
                        }
                        int64_t reg = (int64_t)p.second.field.value;
                        int64_t r = (ac.reg_save.find(reg) == ac.reg_save.end()) ?
                                        reg : ac.reg_save[reg];
                    ...
                    for (const auto &p : ni->opnds) {
                        ...
                        if (ac.nonRegVar.find(v) == ac.nonRegVar.end()) {
                            if (ac.assigned[v] == UNASSIGNED) {
                                ...
                                //In that case we can just allocate any register
                                int64_t reg = find_free_register(ac.lastRead[v], ac);
                                ac.assigned[v] = reg;
                                ac.assignment[reg] = v;
                            } else if (ac.assigned[v] < 0) {
                                //variable v is currently stored in a spill slot
                                int64_t r = find_free_register(ac.lastRead[v], ac);
                                mov_instr(r, ac.assigned[v], ac);
                                //IMPROVEMENT: add r back to savable after all arguments are obtained
                            }
                        }
                    ...
                    #ifdef DEBUG_ASSIGNMENTS
                        for (const auto &p : ni->opnds) {
                            assert((p.second == -1) || (ac.assigned[p.second] >= 0)
                                          || (ac.nonRegVar.find(p.second) != ac.nonRegVar.end()));
                        }
                    #endif
                } else {
                    assert(ni->type() == NEWINSTR_FIXED);

                    //mark all necessary registers as used in the next instruction
                    for (auto &p : ni->def) {
                        if (p.first < 0) continue; //skip immediate operands
                        if (ac.nonRegVar.find(p.first) != ac.nonRegVar.end()) continue;
                            //non-register variable
                        int64_t reg = p.second.getRegister();
                        assert(reg != X86_REG_INVALID);
                        ac.usedNext.insert(reg);
                        if ((p.second.field.type == VAR_MEM_BASE) && (p.second.field.base != X86_REG_INVALID) && (p.second.field.index != X86_REG_INVALID)) {
                    ...
                    for (int v : ni->inputs) {
                        if (ac.nonRegVar.find(v) != ac.nonRegVar.end()) continue;
                        inputRegs.insert(ni->def[v].getRegister());
                    }
                    vector<pair<int, int64_t> > assignedAsOutput;
                    //clear out everything from the registers used and copy in the data needed
                    for (auto &p : ni->def) {
                        if (p.first < 0) continue; //skip immediate operands
                        if (ac.nonRegVar.find(p.first) != ac.nonRegVar.end()) continue;

                        int64_t reg = p.second.getRegister();
                        if (reg == ac.assigned[p.first]) continue;

                        //look for variables or live registers already using this register
                        if (ac.assignment.find(reg) != ac.assignment.end()) {
                            int64_t reloc = find_free_space(ac.lastRead[ac.assignment[reg]], ac);
                            if (reloc != reg) mov_instr(reloc, reg, ac);
                        }
                        if (ac.saved_content.find(reg) != ac.saved_content.end()) {
                            if (ac.lastReadOrig[ac.saved_content[reg]] <= ac.N) {
                                int64_t reloc = find_free_space(ac.lastReadOrig[ac.saved_content[reg]], ac);
                                if (reloc != reg) mov_instr(reloc, reg, ac);
                            } else {
                                //register never used => just save and restore at the start and end
                                ac.relocatedOriginal.insert(reg);
                        ...
                        assert(ac.assignment.find(reg) == ac.assignment.end());
                        assert(ac.saved_content.find(reg) == ac.saved_content.end());
                        ...
                        if (ac.assigned[p.first] == UNASSIGNED) {
                            ...
                            if (inputRegs.find(reg) != inputRegs.end()) {
                                //an input variable already uses the same register, meaning that 
                                //this variable will only be first defined after the instruction
                                //will finish
                    ...
                    //output variables be assigned after the instruction has finished
                ...
                //Return registers that are not read after this point to the free register pool
                while (ac.freeUp.begin()->first == j) {
                    ...
                    if (ac.saved_content.find(r) != ac.saved_content.end()) {
                        if (ac.lastReadOrig[ac.saved_content[r]] > j) {
                            continue;
                            //if the register holds the saved copy of a live register, it's not free
                        } else {
            ...
            //Now give rules to save live registers relocated at any instruction that could fault
            ...
            for (int64_t reg : ac.regsWritten) {
                if (reg <= 0) continue; //spill slots do not have to be saved

                overwrittenReg.insert(reg);
            }

            //check whether flags are to be saved in this sequence: saved iff the actual live
            //definition of eflags is used directly in a machine instruction
            //IMPROVEMENT: only save if the prefetch sequence actually changes it
            bool saveFlags(0);
            Variable EFLAGS_REGISTER = Variable((uint32_t)JREG_EFLAGS);
            VariableState *vs = ac.bb->alive(EFLAGS_REGISTER, ac.pc - ac.bb->instrs[0].pc);
            //if the current live definition is defined earlier in the same block
            if (vs && (vs->block == ac.bb)
                ...
                for (auto *mi : vs->dependants) {
                    ...
                    if (mi->pc > ac.pc) saveFlags = 1;
                }
            }
            if (saveFlags) {
                RewriteRule *h = OPT_RESTORE_FLAGS_rule(ac.bb, ac.pc, ac.rrule_ID++).encode();
                ac.rules.push_back(h);
            }

            #ifdef DEBUG_ASSIGNMENTS
                cerr << ac.rules.size() << " rules generated. Encoded versions:" << endl;
            ...
            cerr << "Failed to generate rules due to too many registers being used" << endl;
        }
        }

        int saveID(0);
        //save&restore registers using the stack to comply with the calling convention
        for (auto it = overwrittenReg.begin(); it != overwrittenReg.end();) {
            ...
            for (int i=0; i<function.entry->size; i++) {
                ...
                if (mi.writeReg(reg)) break;
                if (mi.isReturn()) break;
                if ((mi.opcode == X86_INS_PUSH) && (mi.readReg(reg))) {
                    //reg is saved to the stack in the entry block
                    //assume that this is using the stack to preserve the original state of the
            ...
            cerr << "HINT - Saving register " << getRegNameX86(reg) << " at the start of function " << function.fid << endl;
            //if not saved already, add instruction now to save it
            //IMPROVEMENT: recognize alignment instructions (eg AND rsp, 0xfffffff0) and insert the push instructions after them
            RewriteRule *rule = OPT_SAVE_REG_TO_STACK_rule(function.entry,
                                                     function.entry->instrs[0].pc,
                                                     saveID,
                                                     reg)
                                                         .encode();
        ...
        for (auto &mi : function.minstrs) {
            if (!mi.isReturn()) continue;

            saveID = 100;
            for (auto it = overwrittenReg.rbegin(); it != overwrittenReg.rend(); it++, saveID++)
            {
                int64_t reg = *it;

            cerr << "HINT - Restoring register " << getRegNameX86(reg) << " at return instruction at " << mi.pc << endl;
    ...
    for (int i=0; i<PREFETCH_ERROR_COUNT; i++) {
        if (!PREFETCH_ERROR_COUNTERS[i]) continue;
        cerr << "!!! " << PREFETCH_ERROR_NAMES[i] << " --- " << PREFETCH_ERROR_COUNTERS[i] << " times." << endl;
    }
}
