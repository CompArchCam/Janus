...
static void generateDOALLRules(JanusContext *gc, Loop &loop);
/// Generate rewrite rules for the sub functions in the given loop
static void generateSubFunctionRules(JanusContext *gc, Loop &loop,
                                     Function &func);
static int getEncodedArrayIndex(Loop *loop, Expr var);
...
static void prepareLoopHeader(JanusContext *gc, Loop &loop)
{
    ...
    if (loop.check.size() > 1) {
        fprintf(stderr, "Loop has multiple check blocks, jumpInstructionOpcode "
    ...
    for (auto checkID : loop.check) {
        ...
        header.jumpingGoesBack =
            (cjump->minstr->getTargetAddress() ==
             loop.start->instrs
                 ->pc); // Look whether the jcc jumps to loop start
    }
#ifdef JANUS_X86
    // In dynamic/parallel/arch/x86 emit_init_induction_variable_block we
    // currently require that s1 is not RAX or RDX
    if (header.scratchReg1 == JREG_RAX || header.scratchReg1 == JREG_RDX) {
        if (header.scratchReg0 != JREG_RAX &&
            header.scratchReg0 != JREG_RDX) { // Try swapping s0 and s1
...
static void generateDOALLRules(JanusContext *gc, Loop &loop)
{
    ...
    for (auto bid : loop.init) {
        ...
        if (!bb->fake && bb->lastInstr()->isConditionalJump()) {
            // Check whether the rule will be attached to a conditional jump
            // instruction
    ...
    RegSet
        checkRegisters; // We use these for MEM_RESTORE_CHECK_REG rule insertion
    ...
    for (auto checkID : loop.check) {
        ...
        if (!cmpInstr) {
            cerr << "error in finding compare instruction for cjump " << *cjump
                 << endl;
            continue;
        }

        Variable checkVar; // This is the check variable obtained from the
                           // induction variable
        Variable
        ...
        for (auto phi : cmpInstr->inputs) {
            if (phi->expr && phi->expr->expandedLoopForm) {
                ...
                if (iter) {
                    ...
                    if (iter->checkState)
                        checkVar = (Variable) * (iter->checkState);
        ...
        // In this case, the static analysis will determine (with
        // Loop::getAbsoluteStorage) that the check value is 0x10, not rax So we
        // use that, instead of the second operand of the instruction
        if (checkVar.type != JVAR_UNKOWN)
            condition = checkVar;
        if (condition.type == JVAR_REGISTER)
            checkRegisters.insert(condition.value);

        if (!found) {
            // cerr<<"Loop "<<loop.id<<" compare instruction does not contain
            // induction variable, quit rule generation"<<endl; we relax this
            // condition since this would never be a constant bound
            break;
        }
        ...
        // if condition is a constant or memory location, then emit a rewrite
        // rule to alter the cmp instruction at runtime
        if (condition.type == JVAR_CONSTANT || condition.type == JVAR_MEMORY ||
            condition.type == JVAR_ABSOLUTE) {
            rule =
                RewriteRule(PARA_UPDATE_LOOP_BOUND, cmpInstr->block->instrs->pc,
                            cmpInstr->pc, cmpInstr->id);
            int var_index = 0;
    ...
    for (auto bid : loop.body) {
        BasicBlock *bb = entry + bid;
        // Which of the conditional merge registers have been written to in this
        // block
        ...
        for (int i = 0; i < bb->size; i++) {
            ...
            if (loop.spillRegs.bits & instr.regReads.bits ||
                ...
                rule = RewriteRule(MEM_SCRATCH_REG, bb->instrs->pc, instr.pc,
                                   instr.id);
                rule.reg1 = loop.header.id;
                insertRule(id, rule, bb);
            }
            ...
            // If the same instruction also gets a MEM_SCRATCH_REG, dynamically
            // it's processed before, so that if the check reg is a scratch reg,
            // the MEM_RESTORE_CHECK_REG value stays
            if ((checkRegisters.bits & instr.regReads.bits) &&
                instr.opcode != Instruction::Compare) {
                rule = RewriteRule(MEM_RESTORE_CHECK_REG, bb->instrs->pc,
                                   instr.pc, instr.id);
                // reg0 contains the check register id that we need to spill
                rule.reg0 =
                    (checkRegisters & instr.regReads)
                        .getNextLowest(JREG_RAX); // The regset should contain
                                                  // only one register
                rule.reg1 = loop.header.id;
                insertRule(id, rule, bb);
            }
            ...
            if (loop.registerToConditionalMerge.bits & instr.regWrites.bits) {
                ...
                blockWrittenCondMergeRegs |=
                    (loop.registerToConditionalMerge.bits &
                     instr.regWrites.bits);
            }
        }
        if (blockWrittenCondMergeRegs) {
            rule = RewriteRule(MEM_RECORD_REG_WRITE, bb->instrs->pc,
                               bb->instrs->pc, bb->instrs->id);
        ...
        for (auto &memInstr : bb->minstrs) {
            ...
            // if the stack access is not private variable, then use shared
            // stack
            if (memInstr.mem->type == JVAR_STACK) {
                ...
                if (loop.privateVars.find(stackVar) == loop.privateVars.end() &&
                    ...
                    // MEM_MAIN_STACK rule is inserted at every non-private
                    // stack accesses induction variable stored in private
                    // stacks are assumed to be private too loop bound are also
                    // privatised too
                    rule = RewriteRule(MEM_MAIN_STACK, bb->instrs->pc,
                                       memInstr.instr->pc, memInstr.instr->id);
                    rule.reg1 = loop.header.id;
                    insertRule(id, rule, entry + bid);
                }
            }
        ...
        if (bb->lastInstr()->opcode == Instruction::Call) {
            /* PARA_SUBCALL_START is inserted before performing the function
             * call */
            ...
            /* PARA_SUBCALL_FINISH is inserted at the block where the subcall is
    ...
    if (useStack)
        loop.header.useStack = 1;
    else
        loop.header.useStack = 0;
    ...
    if (useStack)
        loop.header.stackFrameSize = stackFrameSize;
    else
        loop.header.stackFrameSize = 0;
    ...
    for (auto checkBase : loop.arrayToCheck) {
        if (checkBase.vs && checkBase.vs->lastModified) {
            ...
            if (lastModified->opcode == Instruction::Call) {
                ...
                Instruction &next =
                    bb->instrs[lastModified->id - bb->startInstID + 1];
                rule = RewriteRule(MEM_RECORD_BOUNDS, bb->instrs->pc, next.pc,
                                   next.id);
            }
            ...
            // variable state is a function argument, then record it at function
            // entry
    ...
    /* MEM_BOUND_CHECK is inserted before the loop after all the inputs been
     * recorded */
    for (auto checkBase : loop.arrayToCheck) {
    ...
    for (auto reg : copySet) {
        if (jreg_is_simd(reg)) {
            ...
            if (vs != NULL && vs->lastModified) {
                ...
                VECT_RULE *rule = new VECT_RULE(instr->block, instr->pc,
        ...
        for (int i = 0; i < bb.size; i++) {
            ...
            if (instr.isVectorInstruction()) {
                ...
                VECT_CONVERT_rule *rule = new VECT_CONVERT_rule(
                    ...
                    (uint16_t)loop.mainIterator->stride, 0, ALIGNED, 0,
...
uint32_t compileParallelRulesToFile(JanusContext *gc)
{
    ...
    header.ruleInstOffset =
        sizeof(RSchedHeader) + (gc->passedLoop) * sizeof(RSLoopHeader);
    ...
    for (auto profile : loop->encodedVariables) {
        if (profile.type == ARRAY_PROFILE) {
            ...
            if (vp.var == profile.array.base &&
                var.vs->version == profile.version)
                return index;
        }
        index++;
    }
