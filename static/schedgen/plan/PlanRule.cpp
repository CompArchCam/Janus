...
static void generateRulesForEachLoop(JanusContext *gc, Loop &loop)
{
    ...
    for (auto bset : loop.parent->getCFG().blockSplitInstrs) {
        ...
        for (auto sid : bset.second) {
            rule = RewriteRule(APP_SPLIT_BLOCK, bb->instrs->pc,
                               loop.parent->minstrs[sid].pc, sid);
        ...
        for (auto &memInstr : bb->minstrs) {
            ...
            for (auto &opnd :
                ...
                if (loop.privateVars.find(stackVar) != loop.privateVars.end() ||
                    ...
                    // If the instruction uses a private stack variable, we will
                    // use privatization to avoid the dependency
                    instrUsesPrivateVar = true;
                }
            }
            if (!instrUsesPrivateVar) {
                rule = RewriteRule(PROF_MEM_ACCESS, bb->instrs->pc,
    ...
    if (gc->sharedOn) {
        /* PROF_CALL_{START,END} is inserted {before,after} each shared library
         * call */
...
static bool loopNeedProfiling(Loop &loop)
{
...
void generateLoopPlannerRules(JanusContext *gc)
{
    ...
    for (auto &loop : gc->loops) {
        if (loopNeedProfiling(loop)) {
            planFile << loop.id << " " << loop.coverage << " "
                     << loop.total_iteration_count / loop.invocation_count
                     << " " << loop.parent->name << endl;
