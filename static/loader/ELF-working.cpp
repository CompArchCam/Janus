...
void Executable::parseELF64()
{
    ...
    int pltsecSectionIndex = -1;
    int pltGOTSectionIndex = -1;
    ...
    if(dynamicSymbolTableSectionIndex > 0) {
        ...
        if(pltSectionIndex > 0){
            ...
            size += plt.endAddr  - plt.startAddr;
        ...
        if(pltGOTSectionIndex > 0){
            ...
            size += plt.endAddr  - plt.startAddr;
            if(index > 0){ 
               if(pltGOTSectionIndex < index)
                   index  = pltGOTSectionIndex;
            }
            else
               index  = pltGOTSectionIndex;
            cout<<"Section: "<<plt.name<<endl;
        }
                    ...
                    */
                    ...
                    // Loop through entries
                     for (; reltab < reltabend; reltab += entrysize) {
                        ...
                        memcpy(&rel, reltab, entrysize);
#ifdef JANUS_X86
                        if(rel.r_type != R_386_JMP_SLOT && rel.r_type != R_X86_64_GLOB_DAT) continue;
#elif JANUS_AARCH64
                        //R_AARCH64_JUMP_SLOT 1026
                        if(rel.r_type != 1026) continue;
#endif
