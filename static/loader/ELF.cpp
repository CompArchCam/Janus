...
void Executable::parseELF64()
{
    ...
    for (uint32_t i = 0; i < nSections; i++) {
        ...
        if (i == stringTableIndex) {
            sectionStringTable =
                (char *)buffer + (uint32_t)(sectHeader.sh_offset);
    ...
    for (uint32_t i = 0; i < nSections; i++) {
        ...
        GASSERT(sectionNameIndex < sectionStringTableSize,
                "Section name string not found");

        string sectionName = string(sectionStringTable + sectionNameIndex);

        Section section(sectionName, sectHeader.sh_addr,
                        buffer + (uint32_t)(sectHeader.sh_offset),
                        sectHeader.sh_size);

        /* Insert fake-symbols at end of each section
         * to prevent disassemble going unbound */
        Symbol symbol(string("section_end"),
                      sectHeader.sh_addr + sectHeader.sh_size, &section,
                      SYM_NONE);
        symbols.insert(symbol);

        switch (sectHeader.sh_type) {
        case SHT_PROGBITS:
            section.type = SECT_EXE;
            break;
        case SHT_SYMTAB:
        ...
        case SHT_REL:
            ...
            break;
        case SHT_STRTAB:
            section.type = SECT_STRTAB;
            break;
        default:
            section.type = SECT_NONE;
            break;
        }
    ...
    if (symbolTableSectionIndex > 0) {
        ...
        GASSERT(symtabSection.sh_link < nSections,
                "Symbol table section link corrupted");

        char *symtabStringTable =
            (char *)buffer + sectionHeaders[symtabSection.sh_link].sh_offset;
        ...
        for (uint32_t isym = 0; symtab < symtabEnd;
             symtab += symtabEntrySize, isym++) {
            ...
            if (int16_t(sym.st_shndx) > 0) {
                ...
                if (type != STT_SECTION) {
                    Symbol symbol(symbolName, sym.st_value, &mySection,
                                  (SymbolType)type);
                    symbols.insert(symbol);
                } else {
                    Symbol symbol(symbolName, sym.st_value, &mySection,
    ...
    if (dynamicSymbolTableSectionIndex > 0) {
        ...
        Elf64_Shdr &dynSymtabSection =
            sectionHeaders[dynamicSymbolTableSectionIndex];

        GASSERT(dynSymtabSection.sh_link < nSections,
                "Dynamic symbol table section link corrupted");

        char *dynSymtabStringTable =
            (char *)buffer + sectionHeaders[dynSymtabSection.sh_link].sh_offset;
        ...
        for (uint32_t isym = 0; dynSymtab < dynSymtabEnd;
             dynSymtab += dynSymtabEntrySize, isym++) {
        ...
        if (pltSectionIndex > 0) {
            ...
            for (uint32_t i = 0; i < nSections; i++) {
                ...
                string sectionName =
                    string(sectionStringTable + sectHeader.sh_name);
                // parse relocations
                if (sectHeader.sh_type == SHT_REL ||
                    sectHeader.sh_type == SHT_RELA) {
                    char *reltab =
                        (char *)buffer + uint32_t(sectHeader.sh_offset);
                    char *reltabend = reltab + uint32_t(sectHeader.sh_size);
                    uint32_t entrysize = sectHeader.sh_entsize;
                    uint32_t expectedEntrySize =
                        sectHeader.sh_type == SHT_RELA
                            ? sizeof(Elf64_Rela)
                            : // Elf64_Rela, Elf64_Rela
                            sizeof(Elf64_Rela) -
                                wordSize / 8; // Elf64_Rel,  Elf64_Rel
                    ...
                    for (; reltab < reltabend; reltab += entrysize) {
                        ...
                        if (rel.r_type != 1026)
                            continue;
#endif
                        string symbolName = string(dynSymtabStringTable +
                                                   relaTab[rel.r_sym].st_name);

                        PCAddress relocatedAddress = rel.r_offset;

                        Symbol symbol(symbolName, relocatedAddress, &plt,
                                      SYM_RELA);
                        symbols.insert(symbol);
                    }
                }
    ...
    for (auto &sect : sections) {
        if (lastSection) {
            if (sect.startAddr)
                ...
                lastSection->endAddr =
                    lastSection->startAddr + lastSection->size;
        }
        lastSection = &sect;
    }
    // TODO resolve the last section end address
    if (lastSection)
        lastSection->endAddr = lastSection->startAddr;
}
