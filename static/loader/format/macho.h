/****************************    macho.h
 ***************************************** Author:        Agner Fog Date
 *created:  2007-01-06 Last modified: 2008-05-23 Project:       objconv Module:
 *macho.h Description: Header file for definition of data structures in 32 bit
 *Mach-O object file. Also defines class MacSymbolTableBuilder Also defines
 *structures for MacIntosh universal binaries
 *
 * Copyright 2006-2008 GNU General Public License http://www.gnu.org/licenses
 * Parts (c) 2003 Apple public source license
...
#define MAC_NOUNDEFS                                                           \
    0x1 // the object file has no undefined references, can be executed
#define MAC_INCRLINK                                                           \
    0x2 // the object file is the output of an incremental link against a base
        // file and can't be link edited again
#define MAC_DYLDLINK                                                           \
    0x4 // the object file is input for the dynamic linker and can't be staticly
        // link edited again
#define MAC_BINDATLOAD                                                         \
    0x8 // the object file's undefined references are bound by the dynamic
        // linker when loaded.
#define MAC_PREBOUND                                                           \
    0x10 // the file has it's dynamic undefined references prebound.
#define MAC_SPLIT_SEGS                                                         \
    0x20 // the file has its read-only and read-write segments split
#define MAC_LAZY_INIT                                                          \
...
#define MAC_FORCE_FLAT                                                         \
    0x100 // the executable is forcing all images to use flat name space
          // bindings
#define MAC_NOMULTIDEFS                                                        \
    0x200 // this umbrella guarantees no multiple defintions of symbols in its
          // sub-images so the two-level namespace rules can always be used
#define MAC_NOFIXPREBINDING                                                    \
    0x400 // do not have dyld notify the prebinding agent about this executable
#define MAC_PREBINDABLE                                                        \
    0x800 // the binary is not prebound but can have its prebinding redone. only
          // used when MH_PREBOUND is not set
#define MAC_ALLMODSBOUND                                                       \
    0x1000 // indicates that this binary binds to all two-level namespace
           // modules of its dependent libraries. only used when MH_PREBINDABLE
           // and MH_TWOLEVEL are both set
#define MAC_SUBSECTIONS_VIA_SYMBOLS                                            \
    0x2000 // safe to divide up the sections into sub-sections via symbols for
           // dead code stripping
#define MAC_CANONICAL                                                          \
...
#define MAC_LC_REQ_DYLD                                                        \
    0x80000000 // This bit is added if unknown command cannot be ignored
...
#define MAC_LC_PREBOUND_DYLIB                                                  \
    0x10 /* modules prebound for a dynamicly linked shared library */
...
#define MAC_LC_TWOLEVEL_HINTS 0x16 /* two-level namespace lookup rules */
...
#define MAC_SG_HIGHVM                                                          \
    0x1 // the file contents for this segment is for the high part of the
        // VM space, the low part is zero filled (for stacks in core files)
#define MAC_SG_FVMLIB                                                          \
    0x2 // this segment is the VM that is allocated by a fixed VM library,
        // for overlap checking in the link editor
#define MAC_SG_NORELOC                                                         \
    0x4 // this segment has nothing that was relocated in it and nothing
...
struct MAC_section_64 { /* for 64-bit architectures */
    ...
    uint32 reserved3;   // reserved (Note: specified in loader.h, but not in
                        // MachORuntime.pdf)
};
...
#define MAC_S_NON_LAZY_SYMBOL_POINTERS                                         \
...
#define MAC_S_SYMBOL_STUBS                                                     \
    0x8 // section with only symbol stubs, byte size of stub in the reserved2
        // field
#define MAC_S_MOD_INIT_FUNC_POINTERS                                           \
    0x9 // section with only function pointers for initialization
#define MAC_S_MOD_TERM_FUNC_POINTERS                                           \
...
#define MAC_S_GB_ZEROFILL                                                      \
    0xc // zero fill on demand section that can be larger than 4 gigabytes
#define MAC_S_INTERPOSING                                                      \
...
#define MAC_S_ATTR_PURE_INSTRUCTIONS                                           \
    0x80000000 /* section contains only true machine instructions */
#define MAC_S_ATTR_NO_TOC                                                      \
    0x40000000 /* section contains coalesced symbols that are not to be in a   \
                  ranlib table of contents */
#define MAC_S_ATTR_STRIP_STATIC_SYMS                                           \
    0x20000000 /* ok to strip static symbols in this section in files with the \
                  MH_DYLDLINK flag */
#define MAC_S_ATTR_NO_DEAD_STRIP 0x10000000 /* no dead stripping */
#define MAC_S_ATTR_LIVE_SUPPORT                                                \
    0x08000000 /* blocks are live if they reference live blocks */
#define MAC_S_ATTR_SELF_MODIFYING_CODE                                         \
...
#define MAC_SEG_PAGEZERO                                                       \
    "__PAGEZERO" // the pagezero segment which has no protections and catches
                 // NULL references for MH_EXECUTE files
#define MAC_SEG_TEXT "__TEXT" // the tradition UNIX text segment
#define MAC_SECT_TEXT                                                          \
    "__text" // the real text part of the text section no headers, and no
             // padding
#define MAC_SECT_FVMLIB_INIT0                                                  \
    "__fvmlib_init0" // the fvmlib initialization section
#define MAC_SECT_FVMLIB_INIT1                                                  \
...
#define MAC_SECT_COMMON                                                        \
    "__common" // the section common symbols are allocated in by the link editor
...
#define MAC_SEG_LINKEDIT                                                       \
    "__LINKEDIT" // the segment containing all structs created and maintained by
                 // the link editor.  Created with -seglinkedit option to ld(1)
                 // for MH_EXECUTE and FVMLIB file types only
#define MAC_SEG_UNIXSTACK "__UNIXSTACK" // the unix stack segment
#define MAC_SEG_IMPORT                                                         \
    "__IMPORT" // the segment for the self (dyld) modifing code stubs that has
...
#define R_SCATTERED                                                            \
    0x80000000 // mask to be applied to the r_address field of a relocation_info
               // structure to tell that is is really a
               // scattered_relocation_info stucture

struct MAC_relocation_info {
    uint32
        r_address; // offset in the section to what is being relocated (source)
    uint32 r_symbolnum : 24, // symbol table index (0-based) if r_extern == 1 or
                             // section number (1-based) if r_extern == 0
        r_pcrel : 1,  // pc relative. The target address (inline) is already pc
                      // relative
...
struct MAC_scattered_relocation_info {
    uint32 r_address : 24, // offset in the section to what is being relocated
                           // (source)
        r_type : 4,        // if not 0, machine specific relocation type
        r_length : 2,      // 0=byte, 1=word, 2=dword, 3=qword
        r_pcrel : 1, // pc relative. The target address is already pc relative
        r_scattered : 1; // 1=scattered, 0=non-scattered (see above)
    int32 r_value; // target address (without any offset added. The offset is
                   // stored inline in the source)
};
...
/* My interpretation (A Fog):
   ...
   contains the "absolute" address of a first reference point, let's call it
   ref1, in the r_value field. The second record, of type MAC32_RELOC_PAIR
   contains the "absolute" address of a second reference point, ref2, in the
   r_value field. The inline value is the "absolute" address of the relocation
   target minus ref2. ref1 is often = target, but may be any label preceding the
   target. The linker has to add (ref1 - ref2) in image minus (ref1 - ref2) in
   object file to the inline value. The relocation source (the position of the
   inline field) is given in r_address in the first record, relative the the
   section. Non-scattered, absolute, r_extern = 1: r_symbolnum = symbol index
   (0-based) Non-scattered, absolute, r_extern = 0: r_symbolnum = section index,
   inline = absolute address of target? Non-scattered, r_pcrel = 1, r_extern =
   1: r_symbolnum = symbol index (0-based) Inline = source absolute address - 4
...
#define MAC32_RELOC_VANILLA                                                    \
    0 // A generic relocation entry for both addresses contained in data
      // and addresses contained in CPU instructions.
#define MAC32_RELOC_PAIR                                                       \
    1 // The second relocation entry of a pair. Only follows a
      // GENERIC_RELOC_SECTDIFF
#define MAC32_RELOC_SECTDIFF                                                   \
    2 // A relocation entry for an item that contains the difference of
      // two section addresses. This is generally used for position-independent
      // code generation.
#define MAC32_RELOC_PB_LA_PTR                                                  \
    3 // PTR—Arelocation entry for a prebound lazy pointer. This is always
      // a scattered relocation entry. The r_value field contains the
      // non-prebound value of the lazy pointer.
#define MAC32_RELOC_LOCAL_SECTDIFF                                             \
    4 // SECTDIFF—Similar to GENERIC_RELOC_SECTDIFF except that this entry
      // refers specifically to the address in this item. If the address is that
      // of a globally visible coalesced symbol, this relocation entry does not
      // change if the symbol is overridden. This is used to associate stack
      // unwinding information with the object code this relocation entry
      // describes.
...
#define MAC64_RELOC_SIGNED                                                     \
    1 // signed 32-bit displacement with implicit -4 addend
...
#define MAC64_RELOC_SIGNED_1                                                   \
    6 // signed 32-bit displacement with implicit -4 addend and explicit -1
      // addend
#define MAC64_RELOC_SIGNED_2                                                   \
    7 // signed 32-bit displacement with implicit -4 addend and explicit -2
      // addend
#define MAC64_RELOC_SIGNED_4                                                   \
    8 // signed 32-bit displacement with implicit -4 addend and explicit -4
      // addend
...
/* If the type is MAC_N_INDR then the symbol is defined to be the same as
 * another symbol.  In this case the n_value field is an index into the string
 * table of the other symbol's name.  When the other symbol is defined then they
 * both take on the defined type and value. */
...
#define MAC_REFERENCED_DYNAMICALLY                                             \
    0x10 // Must be set for any defined symbol that is referenced by
         // dynamic-loader APIs (such as dlsym and NSLookupSymbolInImage) and
         // not ordinary undefined symbol references. The strip tool uses this
         // bit to avoid removing symbols that must exist: If the symbol has
         // this bit set, strip does not strip it.

#define MAC_N_DESC_DISCARDED                                                   \
    0x20 // Sometimes used by the dynamic linker at runtime in a fully linked
         // image. Do not set this bit in a fully linked image.
//#define MAC_N_DESC_DISCARDED 0x8000

#define MAC_N_NO_DEAD_STRIP                                                    \
    0x20 // When set in a relocatable object file (file type MH_OBJECT) on a
         // defined symbol, indicates to the static linker to never dead-strip
         // the symbol. (Note that the same bit (0x20) is used for two
         // nonoverlapping purposes.)

#define MAC_N_WEAK_REF                                                         \
    0x40 // Indicates that this undefined symbol is aweak reference. If the
         // dynamic linker cannot find a definition for this symbol, it sets the
         // address of this symbol to 0. The static linker sets this symbol
         // given the appropriate weak-linking flags.

#define MAC_N_WEAK_DEF                                                         \
    0x80 // Indicates that this symbol is a weak definition. If the static
         // linker or the dynamic linker finds another (non-weak) definition for
         // this symbol, theweak definition is ignored. Only symbols in a
         // coalesced section (page 21) can be marked as a weak definition.

// Data structure used when sorting symbol table for Mach-O file in
// MacSymbolTableBuilder
template <class TMAC_nlist> struct MacSymbolRecord : public TMAC_nlist {
...
class MacSymbolTableBuilder : public CMemoryBuffer
{
  ...
  public:
    ...
    void AddSymbol(int OldIndex, const char *name, int type, int Desc,
                   int section, MInt value); // Add symbol to list
    void SortList();                         // Sort the list
    int TranslateIndex(
        int OldIndex); // Translate old index to new index, after sorting
    void StoreList(CMemoryBuffer *SymbolTable,
                   CMemoryBuffer *StringTable); // Store sorted list in buffers
...
// Macros listing all word-size dependent structures, used as template parameter
// list
#define MACSTRUCTURES                                                          \
    TMAC_header, TMAC_segment_command, TMAC_section, TMAC_nlist, MInt
#define MAC32STRUCTURES                                                        \
    MAC_header_32, MAC_segment_command_32, MAC_section_32, MAC_nlist_32, int32
#define MAC64STRUCTURES                                                        \
    MAC_header_64, MAC_segment_command_64, MAC_section_64, MAC_nlist_64, int64
