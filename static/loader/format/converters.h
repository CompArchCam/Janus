...
class CCOFF : public CFileBuffer
{
  public:
    ...
    static void
    PrintSegmentCharacteristics(uint32 flags); // Print segment characteristics
    char const *
    GetSymbolName(int8 *Symbol); // Get symbol name from 8 byte entry
    char const *
    GetSectionName(int8 *Symbol); // Get section name from 8 byte entry
    const char *GetFileName(
        SCOFF_SymTableEntry *); // Get file name from records in symbol table
    const char *GetShortFileName(
        SCOFF_SymTableEntry *); // Same as above. Strips path before filename
    char const *GetStorageClassName(uint8 sc); // Get storage class name
    void PublicNames(CMemoryBuffer *Strings, CSList<SStringEntry> *Index,
                     int m); // Make list of public names
    int GetImageDir(
        uint32 n,
        SCOFF_ImageDirAddress
            *dir); // Find address of image directory for executable files
  protected:
    ...
    SCOFF_SymTableEntry
        *SymbolTable;       // Pointer to symbol table (for object files)
    ...
    SCOFF_OptionalHeader
        *OptionalHeader; // Optional header (for executable files)
    SCOFF_IMAGE_DATA_DIRECTORY
        *pImageDirs;     // Pointer to image directories (for executable files)
...
// version
template <class TFileHeader, class TSectionHeader, class TSymbol,
          class TRelocation>
...
class COMF : public CFileBuffer
{
  ...
  protected:
    ...
    CSList<uint32> LocalNameOffset;   // Offset into NameBuffer of segment names
                                      // by name index
    CSList<uint32> SegmentNameOffset; // Offset into NameBuffer of segment names
                                      // by segment index
    CSList<uint32>
        SymbolNameOffset; // Offset into NameBuffer of external symbol names
    ...
    uint32 GetLocalNameO(uint32 i); // Get segment name by converting name index
                                    // offset into NameBuffer
...
// Class for interpreting and dumping Mach-O files. Has templates for 32 and 64
// bit version
template <class TMAC_header, class TMAC_segment_command, class TMAC_section,
          class TMAC_nlist, class MInt>
...
// class CCOF2ELF handles conversion from PE/COFF file to ELF file. Has
// templates for 32 and 64 bit version
template <class TELF_Header, class TELF_SectionHeader, class TELF_Symbol,
          class TELF_Relocation>
class CCOF2ELF : public CCOFF
{
  ...
  protected:
    ...
    void
    MakeSymbolTable(); // Convert subfunction: Symbol table and string tables
    ...
    CArrayBuf<TELF_SectionHeader>
        NewSectionHeaders;         // Buffer for temporary section headers
...
class CCOF2OMF : public CCOFF
{
  ...
  protected:
    ...
    void MakeRelocationsList(); // Make temporary list of relocations (fixups)
                                // and sort it
    ...
    CArrayBuf<SOMFSegmentList>
        SectionBuffer; // Summarize old sections. Translate section index to
                       // segment index
    CArrayBuf<SOMFSymbolList>
        SymbolBuffer; // Translate old symbol index to new public/external index
...
class COMF2COF : public COMF
{
  ...
  protected:
    ...
    void MakeSymbolTable1(); // Make symbol table and string table entries for
                             // file and segments
    void MakeSymbolTable2(); // Make symbol table and string table entries for
                             // external symbols
    void MakeSymbolTable3(); // Make symbol table and string table entries for
                             // public symbols
    void MakeSymbolTable4(); // Make symbol table and string table entries for
                             // communal symbols
    void MakeSymbolTable5(); // Make symbol table and string table entries for
                             // local symbols
    void MakeSections();     // Make sections and relocation tables
    void MakeBinaryFile();   // Putting sections together
    void CheckUnsupportedRecords(); // Make warnings if file containes
    ...
    CMemoryBuffer NewData; // Raw data for each section in new file and its
                           // relocation table
    CSList<uint32> SegmentTranslation; // Translate old segment number to new
                                       // symbol table index
    CSList<uint32> ExtdefTranslation; // Translate old external symbol number to
                                      // new symbol table index
    CSList<SOMFLocalSymbol>
        LocalSymbols; // List for assigning names to unnamed local symbols
    SCOFF_FileHeader NewFileHeader; // New file header
};

// class CELF2COF handles conversion from ELF file to PE/COFF file. Has
// templates for 32 and 64 bit version
template <class TELF_Header, class TELF_SectionHeader, class TELF_Symbol,
          class TELF_Relocation>
class CELF2COF : public CELF<ELFSTRUCTURES>
{
  ...
  protected:
    ...
    void MakeSectionsIndex(); // Convert subfunction: Make section index
                              // translation table
    void
    MakeSections(); // Convert subfunction: Make sections and relocation tables
    void
    MakeSymbolTable(); // Convert subfunction: Symbol table and string tables
...
template <class TELF_Header, class TELF_SectionHeader, class TELF_Symbol,
          class TELF_Relocation, class TMAC_header, class TMAC_segment_command,
          class TMAC_section, class TMAC_nlist, class MInt>
class CELF2MAC : public CELF<ELFSTRUCTURES>
{
  ...
  protected:
    ...
    void
    MakeSymbolTable(); // Convert subfunction: Symbol table and string tables
    void FindUnusedSymbols(); // Convert subfunction: Check if symbols used,
                              // remove unused symbols
    void MakeBinaryFile();    // Convert subfunction: Putting sections together
    // Translate relocations, seperate function for 32 and 64 bits:
    void Elf2MacRelocations(Elf32_Shdr &, MAC_section_32 &,
                            uint32 NewRawDataOffset, uint32 oldsec);
    void Elf2MacRelocations(Elf64_Shdr &, MAC_section_64 &,
                            uint32 NewRawDataOffset, uint32 oldsec);
    ...
    CMemoryBuffer NewStringTable;   // Buffer for building new string table
    CMemoryBuffer
        UnnamedSymbolsTable; // Buffer for assigning names to unnamed symbols
    CArrayBuf<int> NewSectIndex;   // Array of new section indices
    CArrayBuf<MInt> NewSectOffset; // Array of new section offsets
    CArrayBuf<int> OldSymbolScope; // Table of symbol bindings: 0 = local, 1 =
                                   // public, 2 = external
    CArrayBuf<int> OldSymbolUsed;  // Check if symbol is used
    MacSymbolTableBuilder<TMAC_nlist, MInt>
        NewSymTab[3]; // New symbol tables for local, public, external symbols
...
template <class TMAC_header, class TMAC_segment_command, class TMAC_section,
          class TMAC_nlist, class MInt, class TELF_Header,
          class TELF_SectionHeader, class TELF_Symbol, class TELF_Relocation>
class CMAC2ELF : public CMACHO<MACSTRUCTURES>
{
  ...
  protected:
    ...
    void MakeBinaryFile();   // Convert subfunction: Putting sections together
    void
    TranslateAddress(MInt addr, uint32 &section,
                     uint32 &offset); // Translate address to section + offset
    ...
    CArrayBuf<CMemoryBuffer> NewSections; // Buffers for building each section
    CArrayBuf<TELF_SectionHeader>
        NewSectionHeaders;         // Array of temporary section headers
    CArrayBuf<int> NewSectIndex;   // Array of new section indices
...
// class CELF2ELF handles symbol changes in ELF file. Has templates for 32 and
// 64 bit version
template <class TELF_Header, class TELF_SectionHeader, class TELF_Symbol,
          class TELF_Relocation>
class CELF2ELF : public CELF<ELFSTRUCTURES>
{
  ...
  protected:
    ...
    MakeSymbolTable(); // Convert subfunction: Symbol table and string tables
    void
    ChangeSections(); // Convert subfunction: Change section names if needed
    void MakeBinaryFile(); // Convert subfunction: Putting sections together
    uint32 isymtab[2];     // static and dynamic symbol table section number
    uint32 istrtab[4]; // string table section number: symbols, dynamic symbols,
                       // sections, debug
    CMemoryBuffer NewSymbolTable[2]; // Buffers for building new symbol tables:
                                     // static, dynamic
    CMemoryBuffer
        NewStringTable[4]; // Buffers for building new string tables: symbols,
                           // dynamic symbols, sections, debug
    CArrayBuf<uint32>
        NewSymbolIndex;       // Array for translating old to new symbol indices
    uint32 NumOldSymbols;     // Size of NewSymbolIndex table
    uint32 FirstGlobalSymbol; // Index to first global symbol in .symtab
    CFileBuffer ToFile;       // File buffer for modified PE file
};

// class CMAC2MAC handles symbol changes in Mach-O file. Has templates for 32
// and 64 bit version
template <class TMAC_header, class TMAC_segment_command, class TMAC_section,
          class TMAC_nlist, class MInt>
class CMAC2MAC : public CMACHO<MACSTRUCTURES>
{
  ...
  protected:
    ...
    MakeSymbolTable(); // Convert subfunction: Symbol table and string tables
    void
    ChangeSegments(); // Convert subfunction: Change segment names if needed
    void ChangeSections(uint32 HeaderOffset,
                        uint32 Num); // Convert subfunction: Change section
                                     // names and relocation records if needed
    void ChangeImportTable(uint32 FileOffset,
                           uint32 Num); // Convert subfunction: Change symbol
                                        // indices in import table if needed
    void MakeBinaryFile(); // Convert subfunction: Putting sections together
    int NewSymbolIndex(
        int OldIndex); // Convert subfunction: Translate old to new symbol index
    uint32 NewFileOffset(uint32 OldOffset); // Convert subfunction: Translate
                                            // old to new file offset
    MacSymbolTableBuilder<TMAC_nlist, MInt>
        NewSymbols[3]; // Buffers for building new symbol tables: local, public,
                       // external
    CMemoryBuffer NewSymbolTable; // Buffer for building new symbol table
    CMemoryBuffer NewStringTable; // Buffer for building new string table
    CFileBuffer ToFile;           // File buffer for modified PE file
...
class CCOF2ASM : public CCOFF
{
  ...
  protected:
    ...
    void MakeDynamicRelocations(); // Make dynamic base relocations for
                                   // executable files
...
class CELF2ASM : public CELF<ELFSTRUCTURES>
{
  ...
  protected:
    CDisassembler Disasm; // Disassembler
    CArrayBuf<int32>
        SectionNumberTranslate; // Translate section numbers in source file to
                                // section numbers in asm file
    CArrayBuf<uint32> SymbolTableOffset; // Addend to add to symbol number for
                                         // each symbol table
    int64 ImageBase;                     // Image base if executable file
    uint32
        ExeType; // File type: 0 = object, 1 = DLL/shared object, 2 = executable
...
class CMAC2ASM : public CMACHO<MACSTRUCTURES>
{
  ...
  protected:
    ...
    CSList<TMAC_section *>
        ImportSections; // List of sections needing extra symbols: import
                        // tables, literals, etc.
};
...
class COMF2ASM : public COMF
{
  ...
  protected:
    ...
    void
    MakePublicSymbolsTable(); // Make symbol table entries for public symbols
    void MakeCommunalSymbolsTable(); // Make symbol table entries for communal
                                     // symbols
    void MakeGroupDefinitions();     // Make segment group definitions
    void MakeSegmentList();          // Make Segments list in Disasm
    void MakeRelocations(int32 Segment, uint32 RecNum, uint32 SOffset,
                         uint32 RSize,
                         uint8 *SData); // Make relocation list in Disasm
    CDisassembler Disasm;               // Disassembler
    CSList<SOMFSegment> Segments;       // Name, size, etc. of all segments
    CSList<uint32> ExtdefTranslation; // Translate old external symbol number to
                                      // disasm symbol table index
    CSList<uint32> PubdefTranslation; // Translate old public symbol number to
                                      // disasm symbol table index
    CMemoryBuffer SegmentData;        // Binary segment data
    int32 NumSegments;                // Number of segments
    int32 FirstComDatSection; // First COMDAT section. All sections before this
                              // are SEGDEF segments
};
