...
#define PE_F_EXEC                                                              \
    0x0002 // file is executable (no unresolved external references)
...
// Extended structure used internally with virtual address translated to
// section:offset
...
struct SCOFF_ExportDirectory {
    ...
    uint32
        ExportAddressTableRVA; // Image-relative address of export address table
    uint32 NamePointerTableRVA; // Image-relative address of export name pointer
                                // table
...
struct SCOFF_ImportDirectory {
    uint32
        ImportLookupTableRVA; // Image-relative address of import lookup table
    uint32 DateTime;
    uint32 ForwarderChain;
    uint32 DLLNameRVA; // Image-relative address of DLL name string
    uint32
        ImportAddressTableRVA; // Image-relative address of import address table
};

// Import rule/name table entry
struct SCOFF_ImportRuleName {
    uint16 Rule;  // Index into export name pointer table
...
struct SCOFF_BaseRelocationBlock {
    ...
    uint32 BlockSize; // Size of SCOFF_BaseRelocationBlock plus all
                      // SCOFF_BaseRelocation
};
...
#define COFF_REL_BASED_HIGHADJ                                                 \
    4 // Two consecutive records: 16 bits high, 16 bits low
...
struct SCOFF_SectionHeader {
    ...
    uint32 VirtualSize; // size of section when loaded. (Should be 0 for object
                        // files, but it seems to be accumulated size of all
                        // sections)
    uint32 VirtualAddress; // subtracted from offsets during relocation.
                           // preferably 0
...
#define PE_SCN_LNK_REMOVE                                                      \
    0x00000800 // will not be part of the image. object files only
...
#define COFF64_RELOC_REL32_1                                                   \
    0x05 // 32 bit, relative to RIP - 1. For instruction with immediate byte
         // operand
#define COFF64_RELOC_REL32_2                                                   \
    0x06 // 32 bit, relative to RIP - 2. For instruction with immediate word
         // operand
#define COFF64_RELOC_REL32_3 0x07 // 32 bit, relative to RIP - 3. (useless)
#define COFF64_RELOC_REL32_4                                                   \
    0x08 // 32 bit, relative to RIP - 4. For instruction with immediate dword
         // operand
#define COFF64_RELOC_REL32_5 0x09 // 32 bit, relative to RIP - 5. (useless)
#define COFF64_RELOC_SECTION                                                   \
    0x0A // 16-bit section index in file. For debug purpose
#define COFF64_RELOC_SECREL 0x0B  // 32-bit section-relative
#define COFF64_RELOC_SECREL7 0x0C //  7-bit section-relative
#define COFF64_RELOC_TOKEN                                                     \
    0x0D // CLR token = 64 bit absolute virtual address. Inline addend ignored
...
uint32 COFF_PutNameInSymbolTable(SCOFF_SymTableEntry &sym, const char *name,
                                 CMemoryBuffer &StringTable);

// Function to put a name into SCOFF_SectionHeader. Put name in string table
// if longer than 8 characters
void COFF_PutNameInSectionHeader(SCOFF_SectionHeader &sec, const char *name,
                                 CMemoryBuffer &StringTable);
