 ...
 *
 * Copyright 2006-2014 GNU General Public License http://www.gnu.org/licenses
 *****************************************************************************/
#ifndef MAINDEF_H
#define MAINDEF_H

// Program version
#define OBJCONV_VERSION 2.38
...
#elif defined(INT_MAX) && defined(LLONG_MAX) && INT_MAX == 2147483647L &&      \
...
#else
// Compilers supporting C99 or C++0x or C++1x have inttypes.h defining these
// integer types This is the preferred solution:
#include <inttypes.h>
// typedef int8_t         int8;       // Gnu compiler can't convert int8_t to
// char
typedef char int8;       // 8 bit  signed integer
...
static inline uint32 HighDWord(uint64 x) { return (uint32)(x >> 32); }

// Check if compiling for big-endian machine
// (__BIG_ENDIAN__ may not be defined even on big endian systems, so this check
// is not sufficient. A further check is done in CheckEndianness() in main.cpp)
...
#define FILETYPE_INTEL_WPO                                                     \
    0x21 // Object file for whole program optimization, Intel
...
#define LIBTYPE_SHORTNAMES                                                     \
    0x10 // Short member names only, compatible with all systems
#define LIBTYPE_WINDOWS                                                        \
    0x11 // Long member names in "//" member, terminated by 0
#define LIBTYPE_LINUX                                                          \
    0x12 // Long member names in "//" member, terminated by '/'+LF
#define LIBTYPE_BSD_MAC                                                        \
    0x13 // Long member name after header. Length indicated by #1/<length>
