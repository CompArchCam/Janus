...
#define OMF_LNAME_LAST                                                         \
    5 // Last default name. Nondefault names start at OMF_LNAME_LAST + 1
...
union OMF_SLocat {  // Structure of first two bytes of FIXUP subrecord swapped =
                    // Locat field
...
union OMF_SFixData { // Structure of FixData field in FIXUP subrecord of FIXUPP
                     // record
    uint8 b;         // Byte
    struct {
        uint8 Target : 2, // Target method (T=0) or target thread number (T=1)
            P : 1, // 0 = target displacement field present, 1 = displacement is
                   // zero
            T : 1, // 0 = target field present, 1 = target defined by thread
            Frame : 3, // Frame method (F=0) or frame thread (F=1)
            F : 1;     // 0 = target frame field present, 1 = frame defined by
                       // thread
    } s;
};

union OMF_STrdDat { // Structure of Thread Data field in THREAD subrecord of
                    // FIXUPP record
    uint8 b;        // Byte
    struct {
...
struct SOMFRecordPointer {
  public:
    ...
    uint32
    GetNumeric(); // Read word or dword, depending on record type even or odd
    uint32
    GetLength(); // Read 1, 2, 3 or 4 bytes, depending on value of first byte
    char *GetString(); // Read string and return as ASCIIZ string
    void Start(int8 *Buffer, uint32 FileOffset,
               uint32 FileEnd);      // Start scanning through records
    uint8 GetNext(uint32 align = 0); // Get next record
    uint32
    InterpretLIDATABlock(); // Interpret Data block in LIDATA record recursively
    uint32 UnpackLIDATABlock(
        int8 *destination,
        uint32 MaxSize); // Unpack Data block in LIDATA record recursively and
...
class COMFFileBuilder : public CFileBuffer
{
  public:
    ...
    void PutNumeric(uint32); // Put word or dword into buffer, depending on type
                             // being even or odd
    void
    PutString(const char *); // Put ASCII string into buffer, preceded by size
...
struct SOMFSegmentList {
  public:
    ...
    uint32 NewNameI;  // Segment name in new file as index into LNAMES record.
                      // Zero for subsequent entries with same segment name
    SCOFF_SectionHeader *psechdr; // Pointer to old section header
    uint32 Align;                 // Alignment = 2^Align
    uint32 Class;                 // Class in new file
    uint32
        Offset; // Offset of section in old file to first section with same name
    uint32 Size; // Size of section. First record has combined size of all
                 // sections with same name
    uint32 SegmentSize; // Size of segment = combined size of all sections with
                        // same name. Stored only in first section of segment
};
...
struct SOMFRelocation {
  public:
    ...
    int operator<(SOMFRelocation const &x) const
    { // operator < for sorting by CSList::Sort()
        return Section < x.Section ||
               (Section == x.Section && SourceOffset < x.SourceOffset);
    }
};

// Structure for assigning names to unnamed local symbols while converting OMF
// file
struct SOMFLocalSymbol {
    ...
    int operator<(const SOMFLocalSymbol &b) const
    {
        return Segment < b.Segment ||
               (Segment == b.Segment && Offset < b.Offset);
    }
};
