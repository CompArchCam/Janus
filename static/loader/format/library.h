...
uint32 EndianChange(
    uint32); // Convert little-endian to big-endian number, or vice versa
...
class CLibrary : public CFileBuffer
{
  public:
    ...
    // static char *TruncateMemberName(char const*);// Remove path and truncate
    // object file name to 15 characters
    static char *ShortenMemberName(
        char const
            *name); // Truncate library member name to 15 characters and make
                    // unique. The original long name is not overwritten
    static char *
    StripMemberName(char *); // Remove path from library member name. Original
                             // long name is overwritten
    const char *
    GetModuleName(uint32 Index); // Get name of module from index or page index
  protected:
    ...
    void CheckOMFHash(
        CMemoryBuffer &stringbuf,
        CSList<SStringEntry> &index); // Check if OMF library hash table has
                                      // correct entries for all symbol names
    void StartExtracting();           // Initialize before ExtractMember()
    char *ExtractMember(
        CFileBuffer *); // Extract next library member from input library
    char *
    ExtractMemberUNIX(CFileBuffer *); // Extract member of UNIX style library
    char *
    ExtractMemberOMF(CFileBuffer *);  // Extract member of OMF style library
    ...
    void
    InsertMember(CFileBuffer *); // Add next library member to output library
    ...
    void
    MakeSymbolTableUnix(); // Make symbol table for COFF, ELF or MACHO library
    CFileBuffer OutFile;   // Buffer for building output file
    CSList<SStringEntry> StringEntries; // String table using SStringEntry
    CMemoryBuffer
        LongNamesBuffer;        // Buffer for building the "//" longnames member
...
union SOMFHashBlock {
    struct {
        ...
        uint8 Data[OMFBlockSize - OMFNumBuckets -
                   1]; // Contains strings and module indices
    } b;
...
class COMFHashTable
{
  public:
    ...
    int FindString(
        uint32 &ModulePage,
        uint32 &Conflicts); // Search for string. Get number of occurrences,
                            // module, number of conflicting strings
    int InsertString(
        uint16 &ModulePage); // Insert string in hash table. Return 0 if success
    void MakeHashTable(CSList<SStringEntry> &StringEntries,
                       CMemoryBuffer &StringBuffer, CMemoryBuffer &HashTable,
                       CLibrary *Library); // Make hash table
  protected:
