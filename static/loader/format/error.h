...
struct SErrorText {
    ...
    int Status; // bit 0-3 = severity: 0 = ignore, 1 = warning, 2 = error, 9 =
                // abort bit 8   = error number not found
    char const *Text; // Error text
};

// General error routine for reporting warning and error messages to STDERR
// output
class CErrorReporter
{
  public:
    ...
    static SErrorText *
    FindError(int ErrorNumber);   // Search for error in ErrorTexts
    void submit(int ErrorNumber); // Print error message
    void submit(int ErrorNumber,
                int extra); // Print error message with extra info
    void submit(int ErrorNumber, int,
                int); // Print error message with two extra numbers inserted
    void submit(int ErrorNumber,
                char const *extra); // Print error message with extra info
    void submit(int ErrorNumber, char const *,
                char const *); // Print error message with two extra text fields
                               // inserted
    void submit(int ErrorNumber, int,
                char const *); // Print error message with two extra text fields
                               // inserted
    ...
    void
    ClearError(int ErrorNumber); // Ignore further occurrences of this error
  protected:
    ...
    void HandleError(SErrorText *err,
                     char const *text); // Used by submit function
};
