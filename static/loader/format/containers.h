...
void operator>>(
    CFileBuffer &a,
    CFileBuffer &b); // Transfer ownership of buffer and other properties
...
class CMemoryBuffer
{
  public:
    ...
    void SetBuffer(int8 *bufs) { buffer = bufs; }
    ...
    uint32 Push(void const *obj,
                uint32 size); // Add object to buffer, return offset
    uint32
    PushString(char const *s); // Add ASCIIZ string to buffer, return offset
    ...
    template <class TX> TX &Get(uint32 Offset)
    { // Get object of arbitrary type from buffer
        if (Offset >= DataSize) {
            err.submit(2016);
            Offset = 0;
        } // Offset out of range
  ...
  private:
    CMemoryBuffer(
        CMemoryBuffer &); // Make private copy constructor to prevent copying
    uint32 BufferSize;    // Size of allocated buffer ( > DataSize)
  protected:
    int8 *buffer;      // Buffer containing binary data. To be modified only by
                       // SetSize and operator >>
...
static inline void operator<<(CFileBuffer &b, CFileBuffer &a)
{
    a >> b;
...
class CFileBuffer : public CMemoryBuffer
{
  public:
    ...
    static char const *
    GetFileFormatName(int FileType); // Get name of file format type
    ...
    char *SetFileNameExtension(
        const char *f); // Set file name extension according to FileType
  protected:
    void GetOMFWordSize();      // Determine word size for OMF file
    void CheckOutputFileName(); // Make output file name or check that requested
                                // name is valid
};
...
class CTextFileBuffer : public CFileBuffer
{
  public:
    ...
    void PutDecimal(int32 x,
                    int IsSigned = 0); // Write decimal number to buffer
    void PutHex(uint8 x,
                int MasmForm = 0); // Write hexadecimal number to buffer
    void PutHex(uint16 x,
                int MasmForm = 0); // Write hexadecimal number to buffer
    void PutHex(uint32 x,
                int MasmForm = 0); // Write hexadecimal number to buffer
    void PutHex(uint64 x,
                int MasmForm = 0); // Write hexadecimal number to buffer
  ...
  private:
    uint32 PushString(char const *s)
    {
        return 0;
    }; // Make PushString private to prevent using it
};
...
// has a constructor or destructor.
template <class RecordType> class CArrayBuf
{
  private:
  ...
  public:
    ...
    { // Destructor
        if (num)
            delete[] buffer; // Deallocate memory. Will call RecordType
                             // destructor if any
    }
    void SetNum(uint32 n)
    { // Set size of array. May be called only once!
        if (n <= num)
            return; // Already allocated
        if (num) {
            err.submit(
                9004); // Cannot resize because items may have destructors
        } else {
            buffer = new RecordType[n]; // Allocate memory. Will call RecordType
                                        // constructor if any
            if (!buffer) {
    ...
    { // Set all items in array to 0
        memset(buffer, 0,
               num * sizeof(RecordType)); // Warning: overwrites all members of
                                          // RecordType with 0
    }
};
...
template <class RecordType> class CSList : private CMemoryBuffer
{
  public:
    ...
    int32 PushSort(RecordType const &x)
    {
        ...
        int32 RecordsToMove =
            (int32)NumEntries - i; // Number of records to move
    ...
    {
        // Add member to list and keep the list sorted. Avoids duplicate
        // entries. PushUnique will insert x in the list and keep the list
        // sorted. If an entry equal to x already exists in the list then x is
        // not inserted, and the return value will be the index to the existing
        // entry. If no entry equal to x existed then x is inserted and the
        // return value is the index to the new entry. This list must be sorted
        // and without duplicates before calling PushUnique. Operator < must be
        // defined for RecordType.
        int32 i = FindFirst(x); // Find where to insert x
    ...
    void Remove(uint32 index)
    {
        ...
        uint32 RecordsToMove =
            NumEntries - index - 1; // Number of records to move
