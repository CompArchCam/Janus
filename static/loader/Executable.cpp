...
Executable::~Executable() { delete[] buffer; }
...
void Executable::open(JanusContext *jc, const char *filename)
{
    ...
    GSTEP("Reading file \"" << filename << "\" size: " << fileSize << " bytes."
                            << endl);
...
void Executable::liftSymbolToFunction(JanusContext *jc)
{
    ...
    for (auto sit = symbols.begin(); sit != symbols.end(); sit++) {
        // cout <<(*sit).name<<" "<<hex<<(*sit).startAddr<<" type
        // "<<(*sit).type<<" section start "<<(*sit).section->startAddr << " end
        // " << (*sit).section->endAddr <<endl;
        if ((*sit).type == SYM_FUNC || (*sit).type == SYM_RELA) {
            // since the symbols is already sorted, just look for the next
            // different symbol
            auto sit_next = sit;
            sit_next++;
            uint32_t size;
...
void Executable::retrieveHiddenSymbol(Section &section)
{
    ...
    while (cs_disasm_iter(cs_handle, (const uint8_t **)(&flatBuffer),
                          &bufferSize, (uint64_t *)&pc, instr)) {
...
bool compareProc(Symbol &a, Symbol &b) { return (a.startAddr) < (b.startAddr); }
