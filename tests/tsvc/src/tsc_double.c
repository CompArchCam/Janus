...
__attribute__ ((aligned(32))) TYPE X[lll],Y[lll],Z[lll],U[lll],V[lll];


//double* __restrict__ array;
double array[LEN2*LEN2] __attribute__((aligned(32)));

double x[LEN] __attribute__((aligned(32)));
double temp;
int temp_int;


__attribute__((aligned(32))) double a[LEN],b[LEN],c[LEN],d[LEN],e[LEN],
                                   aa[LEN2][LEN2],bb[LEN2][LEN2],cc[LEN2][LEN2],tt[LEN2][LEN2];


int indx[LEN] __attribute__((aligned(32)));


double* __restrict__ xx;
double* yy;

int dummy(double[LEN], double[LEN], double[LEN], double[LEN], double[LEN], double[LEN2][LEN2], double[LEN2][LEN2], double[LEN2][LEN2], double);

int dummy_media(short[], char[], int);

int set1d(double arr[LEN], double value, int stride)
{
    if (stride == -1) {
        for (int i = 0; i < LEN; i++) {
            arr[i] = 1. / (double) (i+1);
...
int set1ds(int _n, double arr[LEN], double value, int stride)
{
    if (stride == -1) {
        for (int i = 0; i < LEN; i++) {
            arr[i] = 1. / (double) (i+1);
...
int set2d(double arr[LEN2][LEN2], double value, int stride)
{
...
int s151s(double a[LEN], double b[LEN],  int m)
{
    for (int i = 0; i < LEN-1; i++) {
        a[i] = a[i + m] + b[i];
    }
...
int s152s(double a[LEN], double b[LEN], double c[LEN], int i)
{
    a[i] += b[i] * c[i];
    return 0;
}
...
void set(int* ip, double* s1, double* s2){
    xx = (double*) memalign(16, LEN*sizeof(double));
...
int main(){
    ...
    int n1 = 1;
    int n3 = 1;
    int* ip = (int *) memalign(16, LEN*sizeof(double));
    double s1,s2;
    set(ip, &s1, &s2);
    s122(n1,n3);
    s123();
