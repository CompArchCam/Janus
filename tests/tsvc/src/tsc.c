...
float array[LEN2*LEN2] __attribute__((aligned(32)));

float x[LEN] __attribute__((aligned(32)));
float temp;
int temp_int;


__attribute__((aligned(32))) float a[LEN],b[LEN],c[LEN],d[LEN],e[LEN],
...
//          a[i] = b[i] + c[i];
    ...
    s = (end.tv_sec-start.tv_sec) + (end.tv_nsec-start.tv_nsec) / 1.0e9; 
    printf("S000\t %.2f \t\t", s);;
    check(12);
