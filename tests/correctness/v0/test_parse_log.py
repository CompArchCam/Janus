...
def get_correct_selected_loops(test_name):
    if (test_name == "2mm"): 
        return "65 68 " #Notice the space after the loops, and loops being in ascending order
    elif (test_name == "3mm"):
        return "142 145 "
...
def print_all_test_summary():
    ...
    expr3 = re.compile(r"Final selected DOALL loops: ((\d|\s)*)\n")
    total_test_count = 0
    successful_test_count = 0
    successful_analyze_count = 0 #This checks if the static analyzer crashed
    for line in file:
        ...
        m = expr3.findall(line)
        if (len(m) >= 1): successful_analyze_count +=1

    print(f"{bcolors.BOLD}------     SUMMARY     ------{bcolors.ENDC}");
    print(f"{bcolors.BOLD}Count of run tests: {total_test_count}{bcolors.ENDC}")
    successful_test_count = min(successful_test_count, successful_analyze_count)
